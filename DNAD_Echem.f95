...
! Need to redefine all double precision as real
...
module MOD_Echem
  ...
  real :: cimax                                    ! maximum solid-state concentration [=] mol/cm3
  real :: alpha_a = 0.5, alpha_c = 0.5             ! cathodic and anodic charge transfer coefficients
  real :: cbulk = 0.001
...
contains
  ...
  FUNCTION OCP(c00, cii) Result(OpenCircuitVoltage)
    ...
    TYPE(DUAL), INTENT (IN) :: c00, cii                 ! c00 - electrolyte conc
                                                        ! cii - solid-state conc
    TYPE(DUAL) :: theta                                 ! cii / cimax [0,1]
    TYPE(DUAL) :: Vint

    real :: U_ref
    real, DIMENSION(0:20) :: AK

          theta = cii

          U_ref =  2.767060050604

          AK = (/ -0.000328953283640, &
                   0.000057047758870, &
                  -0.000214750167832, &
                   0.000241771585756, &
                   0.001818560132395, &
                  ...
                  -0.237501333152191, &
                   0.260212063257241, &
                   0.052050020643885 /) * 1.0d3


            Vint = 0.0
            if (theta == 0.5) then
              Vint = 0.0 ! ((2.0*theta-1.0)**(kk+1) - (2.0*theta*kk*(1.0-theta)) * (2.0*theta-1.0)**(kk-1)) = 0 when theta = 0.5
            else
              do kk=0,20
...
PROGRAM CircleArea
  ...
  c0 = DUAL(1.0d-3, (/1.0, 0.0/))
  ci = DUAL(0.4, (/0.0, 1.0/))
  xx = OCP(c0, ci)

  ! check that the derivatives are calculated accurately
  WRITE(*,*) xx, Rigc*Temp/Fconst/c0%x, Rigc*Temp/Fconst * (1.0/(ci%x**2 - ci%x))

  ! how to access the derivative values when N > 1
  WRITE(*,*) xx%x, xx%dx(1), xx%dx(2)
