...
module user_input
  ...
  real, parameter :: K_ph = 10**(-13.74)
  real, parameter :: K_5  = 10**(6.05)
  real, parameter :: K_6  = 10**(9.73)

  real, parameter :: K_CO2 = 10**(-1.491)       ! [mol/(L * bar)]
  real, parameter :: P_CO2 = 1.013              ! [bar]

  ! KHCO_3 --> K^+ + HCO_3^-
  real, parameter :: cbulk_K   = 0.5e-3                    ! [mol/cm3]
  real, parameter :: cbulk_CO2 = K_CO2 * P_CO2 *1e-3       ! [mol/cm3]
  real, parameter :: cbulk_H   = cbulk_CO2*1e3 / (K_5*cbulk_K*1e3) * 1e-3
...
module homogeneous_reactions_mod
  use dnadmod
  use user_input

contains

  function conc_OH(conc_H)                  result(OH)
    ! K_ph = [H+][OH-]
    type(dual)              :: OH
    type(dual), intent(in)  :: conc_H

    OH = K_ph / conc_H

  end function conc_OH

  function conc_HCO3(conc_H, conc_CO2)      result(HCO3)
    ! K_5 = [CO2]/([HCO3-][H+])
    type(dual)              :: HCO3
    type(dual), intent(in)  :: conc_H, conc_CO2

    HCO3 = conc_CO2 / (K_5 * conc_H)

  end function conc_HCO3

  function conc_CO3(conc_H, conc_CO2)      result(CO3)
    ! K_6 = [HCO3-]/([CO3^{-2}][H+])
    type(dual)              :: CO3
    type(dual), intent(in)  :: conc_H, conc_CO2
    type(dual)              :: HCO3

    HCO3 = conc_HCO3(conc_H, conc_CO2)
    CO3 = HCO3 / (K_6 * conc_H)

  end function conc_CO3


  ! function conc_CO3(conc_H, conc_CO2)      result(CO3)
  !   ! combine equations 5 and 6
  !   type(dual)  :: CO3
  !   type(dual)  :: conc_H, conc_CO2
  !
  !   CO3 = conc_CO2 / (K_5 * K_6 * conc_H**2)
  !
  ! end function conc_CO3

end module homogeneous_reactions_mod
...
! (4)     0 = sum_i (z_i * c_i)       ! electroneutrality
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    type(dual) :: c_CO2, c_H, c_K, Phi_2, c_HCO3, c_CO3, c_OH
    type(dual) :: dc_CO2dx, dc_Hdx, dc_Kdx, dPhi2dx, dc_HCO3dx, dc_CO3dx, dc_OHdx
    ...
    Phi_2     = c_vars_dual(4)
    c_OH      = conc_OH(c_H)
    c_HCO3    = conc_HCO3(c_H, c_CO2)
    c_CO3     = conc_CO3(c_H, c_CO2)

    dc_CO2dx  = dcdx_vars_dual(1)
    dc_Hdx    = dcdx_vars_dual(2)
    dc_Kdx    = dcdx_vars_dual(3)
    dPhi2dx   = dcdx_vars_dual(4)
    ! use the chain rule to calculate the conc gradients of OH, HCO3, CO3
    ! d(c_i)/dx = dc_CO2/dx * d(c_i)/d(c_CO2) + dc_H/dx * d(c_i)/d(c_H)
    dc_OHdx   = dc_CO2dx * c_OH%dx(1)   + dc_Hdx * c_OH%dx(2)
    dc_HCO3dx = dc_CO2dx * c_HCO3%dx(1) + dc_Hdx * c_HCO3%dx(2)
    dc_CO3dx  = dc_CO2dx * c_CO3%dx(1)  + dc_Hdx * c_CO3%dx(2)
    ...
    Flux_(3) = Flux_K
    Flux_(4) = 0.0      ! No flux equation for electroneutrality
  ...
  function RXN(c_vars_dual) result(Rxn_)
    ...
    Rxn_ = 0.0
    ! electroneutrality
    Rxn_(4) = z_CO2 * c_CO2 + z_H * c_H + z_K * c_K + &
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    BC_WEST_(1) = flux_temp(1) - 0.0 ! flux_temp(1) + R_1 = 0.0
    BC_WEST_(2) = flux_temp(2) - 0.0  ! flux_temp(2) + 2*R2 + R3 + R4 = 0.0
    BC_WEST_(3) = flux_temp(3) - 0.0  ! N_K+ = 0
    BC_WEST_(4) = rxn_temp(4)  - 0.0  ! electroneutrality
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(2) = c_H   - cbulk_H
    BC_EAST_(3) = c_K   - cbulk_K
    BC_EAST_(4) = Phi_2 - 0.0
...
program unsteady
  ...
  integer :: it, j
  real, dimension(N*2)           :: dx_array = 0.0

  type(dual) :: c_OH, c_CO3, c_HCO3, H_dual, CO2_dual

  H_dual = dual(cbulk_H, dx_array)
  CO2_dual = dual(cbulk_CO2, dx_array)

  c_OH = conc_OH(H_dual)
  c_CO3 = conc_CO3(H_dual, CO2_dual)
  c_HCO3 = conc_HCO3(H_dual, CO2_dual)

  call system_clock(t1,clock_rate,clock_max)
  call initial_condition()

  print*, cbulk_K, cbulk_CO2, cbulk_H, c_OH%x, c_CO3%x, c_HCO3%x
  stop
