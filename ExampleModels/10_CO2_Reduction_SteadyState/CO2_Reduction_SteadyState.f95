...
module user_input
  ...
  integer, parameter :: maxIterations = 200     ! Number of time steps
  real               :: xmax = 1e-2                      ! [cm]
  real               :: error = 1e3
  real               :: tol   = 1e-15

  real, parameter :: PI     = 4.0 * ATAN(1.0)   ! pi - Geometric constant
  real, parameter :: Temp   = 298               ! [K]
  real, parameter :: Rigc   = 8.314             ! [J/(mol*K)]
  real, parameter :: Fconst = 96485             ! [C/mol]

  ! Mathematical Modeling of CO2 Reduction to CO in Aqueous Electrolytes
  ! I. Kinetic Study on Planar Silver and Gold Electrodes
  ! Charles Delacourt, Paul L. Ridgway, and John Newman
  ! HCO3- + H+    <--> CO2 + H2O      (5)   K_5  = 1e6.05
  ! CO3^{-2} + H+ <--> HCO3-          (6)   K_6  = 1e9.73
  ! H2O           <--> OH- + H+       (7)   K_pH = 1e-13.74

  real, parameter :: K_pH = 10**(-13.74) * 1e-6   ! [(mol/cm3)^2]
  real, parameter :: K_5  = 10**(6.05) * 1e3      ! [cm3/mol]
  real, parameter :: K_6  = 10**(9.73) * 1e3      ! [cm3/mol]

  real, parameter :: K_CO2 = 10**(-1.491) * 1e-3  ! [mol/(cm3 * bar)]
  real, parameter :: P_CO2 = 1.013                ! [bar]

  ! equilibrated RDE reactions
  real, parameter :: P_CO = 1.0
  real, parameter :: P_H2 = 1.0
  real, parameter :: n_CER = 2.0
  real, parameter :: n_HER = 2.0

  ! KHCO_3 --> K^+ + HCO_3^-
  real, parameter :: cbulk_K   = 0.5e-3                    ! [mol/cm3]
  real, parameter :: cbulk_CO2 = K_CO2 * P_CO2             ! [mol/cm3]
  real, parameter :: cbulk_H   = 1e-7 * 1e-3

  character(len=65) :: geometry = 'Rectangular'   ! system Geometry: Rectangular, Cylindrical, Spherical

  ! ion charges
  real, parameter :: z_CO2  =  0.0
  real, parameter :: z_H    = +1.0
  real, parameter :: z_K    = +1.0
  real, parameter :: z_Na   = +1.0
  real, parameter :: z_OH   = -1.0
  real, parameter :: z_HCO3 = -1.0
  real, parameter :: z_CO3  = -2.0
  real, parameter :: z_ClO4 = -1.0

  ! Diffusion Coefficients: units [cm^2/s]
  real, parameter :: D_CO2  = 1.97e-5
  real, parameter :: D_H    = 9.312e-5
  real, parameter :: D_K    = 1.957e-5
  real, parameter :: D_Na   = 1.334e-5
  real, parameter :: D_OH   = 5.26e-5
  real, parameter :: D_HCO3 = 1.105e-5
  real, parameter :: D_CO3  = 0.955e-5
  real, parameter :: D_ClO4 = 1.792e-5

  real :: Phi_1, RDE_current
...
module write_data_mod
  ...
  real :: c_CO2, c_H, c_K, c_OH, c_HCO3, c_CO3, Phi_2
  logical :: Converge_BOOL

  character(len=65) :: header_fmt = '(1(A12,1X),  20(A15,1X)) '
  character(len=65) :: data_fmt   = '(1(L12,1X),  20(ES15.5,1X))'
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then ! write headers
      write(*, header_fmt) 'Convergence',  '[CO2]',  '[H+]', '[HCO3-]', '[CO3-2]', '[OH-]', '[K+]', 'Phi_2', 'Phi_1', 'RDE_Current'
      write(*, header_fmt) 'True/False', 'mol/L',  'mol/L', 'mol/L',  'mol/L', 'mol/L',  'mol/L', 'Volts', 'Volts', 'mA/cm2'
      ...
      c_K       = cprev(3,1)*1e3
      c_OH      = cprev(4,1)*1e3
      c_HCO3    = cprev(5,1)*1e3
      c_CO3     = cprev(6,1)*1e3
      Phi_2     = cprev(7,1)

      write(*, data_fmt) Converge_BOOL,    c_CO2,  c_H, c_HCO3, c_CO3, c_OH, c_K, Phi_2, Phi_1, RDE_current*1e3
  ...
  subroutine write_positional_information_to_file(it)
    ...
    open(56, file = 'Conc_Position.txt', status = 'unknown')

    if (it == 0) then
      write(56, header_fmt) 'Convergence',  'Position', '[CO2]',  '[H+]', &
      & '[HCO3-]', '[CO3-2]', '[OH-]', '[K+]', 'Phi_2', 'Phi_1', 'RDE_Current'
      write(56, header_fmt) 'True/False',   'um'      , 'mol/L',  'mol/L', &
      & 'mol/L',  'mol/L', 'mol/L',  'mol/L', 'Volts', 'Volts', 'mA/cm2'
      ...
      do j = 1, NJ                                    !
        ...
        c_K       = cprev(3,j)*1e3
        c_OH      = cprev(4,j)*1e3
        c_HCO3    = cprev(5,j)*1e3
        c_CO3     = cprev(6,j)*1e3
        Phi_2     = cprev(7,j)
                                                      !           !
        write(56, data_fmt) Converge_BOOL,   xx(j)*1e4,    c_CO2,  c_H, c_HCO3, c_CO3, c_OH, c_K, Phi_2, Phi_1, RDE_Current*1e3
...
! Equations
! (1) d(c_CO2)  = -d/dx (N_CO2 + N_HCO3 + N_CO3)
! (2) d(c_CO2)  = -d/dx (N_H - 2*N_CO3 - N_HCO3 - N_OH)
! (3) d(c_K)    = -d/dx (N_K)
! (4)     0 = sum_i (z_i * c_i)       ! electroneutrality

  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ! (i)   N_i = -D dc/dx - z_i u_i F c d(Phi)/dx
    ! (1)   N_CO2
    ! (2)   N_H+
    ! (3)   N_K+
    ! (4)   electroneutrality
    ...
    type(dual) :: c_CO2, c_H, c_K, c_HCO3, c_CO3, c_OH, Phi_2
    type(dual) :: dc_CO2dx, dc_Hdx, dc_Kdx, dc_HCO3dx, dc_CO3dx, dc_OHdx, dPhi2dx
    type(dual) :: Flux_CO2, Flux_H, Flux_K, Flux_OH, Flux_HCO3, Flux_CO3

    c_CO2     = c_vars_dual(1)
    c_H       = c_vars_dual(2)
    c_K       = c_vars_dual(3)
    c_OH      = c_vars_dual(4)
    c_HCO3    = c_vars_dual(5)
    c_CO3     = c_vars_dual(6)
    Phi_2     = c_vars_dual(7)

    dc_CO2dx  = dcdx_vars_dual(1)
    dc_Hdx    = dcdx_vars_dual(2)
    dc_Kdx    = dcdx_vars_dual(3)
    dc_OHdx   = dcdx_vars_dual(4)
    dc_HCO3dx = dcdx_vars_dual(5)
    dc_CO3dx  = dcdx_vars_dual(6)
    dPhi2dx   = dcdx_vars_dual(7)

    Flux_CO2  = -D_CO2*dc_CO2dx &
              & - z_CO2 * D_CO2/(Rigc*Temp)*Fconst * c_CO2 * dPhi2dx

    Flux_H    = -D_H*dc_Hdx &
              & - z_H * D_H/(Rigc*Temp)*Fconst * c_H * dPhi2dx

    Flux_K    = -D_K*dc_Kdx &
              & - z_K * D_K/(Rigc*Temp)*Fconst * c_K * dPhi2dx

    Flux_OH   = -D_OH*dc_OHdx &
              & - z_OH * D_OH/(Rigc*Temp)*Fconst * c_OH * dPhi2dx

    Flux_HCO3 = -D_HCO3*dc_HCO3dx &
              & - z_HCO3 * D_HCO3/(Rigc*Temp)*Fconst * c_HCO3 * dPhi2dx

    Flux_CO3  = -D_CO3*dc_CO3dx &
              & - z_CO3 * D_CO3/(Rigc*Temp)*Fconst * c_CO3 * dPhi2dx


    Flux_(1) = Flux_CO2 + Flux_HCO3 + Flux_CO3
    Flux_(2) = Flux_H - 2*Flux_CO3 - Flux_HCO3 - Flux_OH
    Flux_(3) = Flux_K
    Flux_(4) = 0.0      ! No flux for equilibrium expressions
    Flux_(5) = 0.0
    Flux_(6) = 0.0
    Flux_(7) = 0.0      ! No flux equation for electroneutrality
  ...
  function RXN(c_vars_dual) result(Rxn_)
    ...
    Rxn_(2) = 0.0
    Rxn_(3) = 0.0
    Rxn_(4) = K_6 * c_CO3 * c_H - c_HCO3
    Rxn_(5) = K_5 * c_H * c_HCO3 - c_CO2
    Rxn_(6) = K_pH - c_OH * c_H
    Rxn_(7) = z_CO2 * c_CO2 + z_H * c_H + z_K * c_K + &
            & z_OH * c_OH + z_HCO3 * c_HCO3 + z_CO3 * c_CO3


  end function RXN

  function ACCUM(c_vars_dual) result(Accum_)
    ! Steady state so d/dt = 0
    type(dual), dimension(N)              :: Accum_
    type(dual), dimension(N), intent(in)  :: c_vars_dual

    ! type(dual) :: c_CO2, c_H, c_K
    !
    ! c_CO2     = c_vars_dual(1)
    ! c_H       = c_vars_dual(2)
    ! c_K       = c_vars_dual(3)
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    ! (5)
    ! (6)
    ! (7)   rxn_temp(7)           Electroneutrality
    type(dual), dimension(N)               :: BC_WEST_
    type(dual), dimension(N), intent(in)   :: c_vars_dual, dcdx_vars_dual
    type(dual), dimension(N)               :: flux_temp, rxn_temp

    type(dual) :: c_CO2, c_H, c_K, Phi_2
    type(dual) :: surf_rxn_1, surf_rxn_2, surf_rxn_3, surf_rxn_4
    type(dual) :: eta_1, eta_3

    real :: U_1, U_3, k_1, k_3, alpha_c1, alpha_c3
    ...
    U_1 = -0.103
    eta_1 = Phi_1 - U_1 - Phi_2
    ! eta_1 = Phi_1 - Phi_2
    k_1 = 5.8e-9
    alpha_c1 = 0.25

    U_3 = 0.0
    eta_3 = Phi_1 - U_3 - Phi_2
    ! eta_3 = Phi_1 - Phi_2
    k_3 = 8.3e-13
    alpha_c3 = 0.20

    ! equilibrated reactions
    surf_rxn_1 = P_CO/(c_CO2 * c_H**2) - exp(-n_CER * Fconst / (Rigc*Temp) * (Phi_1 - U_1 - Phi_2))
    surf_rxn_3 = P_H2/(c_H**2) - exp(-n_HER * Fconst / (Rigc*Temp) * (Phi_1 - Phi_2))

    ! only CER
    surf_rxn_3 = 0.0
    BC_WEST_(1) = surf_rxn_1
    BC_WEST_(2) = flux_temp(2) - 2*flux_temp(1)
    RDE_current = Fconst * (2*flux_temp(1)%x)

    ! only HER
    ! BC_WEST_(1) = flux_temp(1) - 0.0
    ! BC_WEST_(2) = surf_rxn_3

    ! Tafel expressions
    ! surf_rxn_1 = k_1 * c_CO2 * exp(-alpha_c1 * Fconst * eta_1/(Rigc*Temp))
    ! surf_rxn_3 = k_3 * exp(-alpha_c3 * Fconst * eta_3/(Rigc*Temp))

    surf_rxn_2 = surf_rxn_1
    surf_rxn_4 = surf_rxn_3

    ! RDE_current = Fconst * (surf_rxn_1%x + surf_rxn_2%x + surf_rxn_3%x + surf_rxn_4%x)

    !
    ! BC_WEST_(1) = flux_temp(1) + surf_rxn_1
    ! BC_WEST_(2) = flux_temp(2) + 2*flux_temp(1)
    ! BC_WEST_(2) = flux_temp(2) + 2*surf_rxn_2 + surf_rxn_3 + surf_rxn_4
    BC_WEST_(3) = flux_temp(3) - 0.0  ! N_K+ = 0
    BC_WEST_(4) = rxn_temp(4)   ! equilibrium expressions
    BC_WEST_(5) = rxn_temp(5)
    BC_WEST_(6) = rxn_temp(6)
    BC_WEST_(7) = rxn_temp(7)   ! electroneutrality
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    ! (2)   c_K   = cbulk_K
    ! (3)   Phi_2 = 0.0
    ! (4)   rxn_temp(4)           equation 6 equilibrium
    ! (5)   rxn_temp(5)           equation 5 equilibrium
    ! (6)   rxn_temp(6)           pH equilibrium
    ! (7)   rxn_temp(7)           Electroneutrality
    type(dual), dimension(N)               :: BC_EAST_
    type(dual), dimension(N), intent(in)   :: c_vars_dual, dcdx_vars_dual
    type(dual), dimension(N)               :: flux_temp, rxn_temp

    type(dual) :: c_CO2, c_K, Phi_2
    ...
    BC_EAST_(1) = c_CO2 - cbulk_CO2
    BC_EAST_(2) = c_K   - cbulk_K
    BC_EAST_(3) = Phi_2 - 0.0
    BC_EAST_(4) = rxn_temp(4)
    BC_EAST_(5) = rxn_temp(5)
    BC_EAST_(6) = rxn_temp(6)
    BC_EAST_(7) = rxn_temp(7)
...
program steady
  ...
  integer :: it = 0, j, pot, pot_len
  real    :: Pot_high, Pot_low, step_size
  ...
  do pot = 0, pot_len
    Phi_1 = Pot_high + (Pot_low - Pot_high)/pot_len * pot
    ...
    error = 1e3
    it = 0
    do while ((error > tol ).AND.( it <= maxIterations))
...
subroutine initial_condition()
  ...
  do j = 1, NJ
    ...
    cprev(2,j) = cbulk_H
    cprev(3,j) = cbulk_K
    cprev(4,j) = K_pH/cbulk_H     ! OH
    cprev(5,j) = cbulk_K          ! HCO3
    cprev(6,j) = 0.0              !
    cprev(7,j) = 0.0
  end do
