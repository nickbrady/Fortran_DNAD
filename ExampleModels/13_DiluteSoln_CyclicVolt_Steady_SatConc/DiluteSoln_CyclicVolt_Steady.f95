...
module user_input
  ...
  real :: c0_sat = 2e-3                      ! 3 M
  real :: i_applied_cm2 = -1e-3
  real :: Delta_V

  character(len=65) :: geometry = '__GEOMETRY__'   ! system Geometry: Rectangular, Cylindrical, Spherical
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Iters', 'DeltaV', 'Conc', 'Phi_1_1', 'Phi_1_NJ', 'DeltaV', 'i_rxn_1', 'i_rxn_NJ'
      write(*, header_fmt) '###',   'Volts'    , 'mol/L', 'Volts', 'Volts', 'Volts'

    else
      c0 = cprev(1,NJ)*1e3
      Phi_1_1 = cprev(2,1)
      Phi_1_NJ = cprev(2,NJ)
      i_rxn_1  = Butler_Volmer_Rxn(cprev(1,1),cprev(2,1))
      i_rxn_NJ = Butler_Volmer_Rxn(cprev(1,NJ),cprev(2,NJ))

      write(*, data_fmt) it, Delta_V, c0,  Phi_1_1, Phi_1_NJ, Phi_1_1 - Phi_1_NJ, &
      & i_rxn_1, i_rxn_NJ
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Iters', 'DeltaV', 'Position', 'Conc', 'Phi_1', 'i_rxn'
      write(56, header_fmt) '###',   'Volts'    , 'cm'      , 'mol/L',  'Volts', 'A/cm2'
                                                    !
    else                                         !
      ...
      do j = 1, NJ                                    !
        c0 = cprev(1,j) * 1e3                         !
        Phi_1 = cprev(2,j)
        i_rxn = -Butler_Volmer_Rxn(cprev(1,NJ),cprev(2,NJ))
                                                      !
        write(56, data_fmt) it, Delta_V, xx(j),    c0, Phi_1, i_rxn!, Phi_2
...
module echem_transport_mod
  ...
  contains
    ...
    function Diffusion_Coef(c_vars_dual)    Result(diff_coef_)
      ...
      if ('__diff_coef__' == 'Constant') then
        diff_coef_%dx = 0.0
        diff_coef_%x  = D_Li_0

      else if ('__diff_coef__' == 'Variable') then
        c0 = c_vars_dual(1)
...
!*******************************************************************************
    function exchange_current_density_dual(c0)  result(ex_curr)
      ...
      ex_curr = Fconst * k_rxn * c0**alpha_a * (c0_sat - c0)**alpha_c
...
!*******************************************************************************
    function OpenCircuitPotential_dual(c0)     result(OCP)
      ...
      OCP = U_ref + Rigc*Temp/Fconst * log(c0 / (c0_sat - c0))
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    c0_1     = c_vars_dual(1)
    c0_NJ    = cprev(1,NJ)
    Phi_1_1  = c_vars_dual(2)
    Phi_1_NJ = Phi_1_1 - Delta_V

    ! BC_WEST_(1) = flux_temp(1) - i_applied_cm2/Fconst
    BC_WEST_(1) = flux_temp(1) - Butler_Volmer_Rxn(c0_1, Phi_1_1)/Fconst
    ! conservation of current at each electrode
    BC_WEST_(2) = Crx_Area(1,1) *Butler_Volmer_Rxn(c0_1, Phi_1_1) + &
                  Crx_Area(2,NJ)*Butler_Volmer_Rxn(c0_NJ, Phi_1_NJ)
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    Phi_1_1  = Delta_V + c_vars_dual(2)
    Phi_1_NJ = c_vars_dual(2)

    ! BC_EAST_(1) = flux_temp(1) - i_applied_cm2/Fconst
    BC_EAST_(1) = flux_temp(1) + Butler_Volmer_Rxn(c0_NJ, Phi_1_NJ)/Fconst
    BC_EAST_(2) = Crx_Area(1,1) *Butler_Volmer_Rxn(c0_1, Phi_1_1) + &
                  Crx_Area(2,NJ)*Butler_Volmer_Rxn(c0_NJ, Phi_1_NJ)
...
program unsteady
  ...
  real :: step_size = 1.0, tol = 1e-7, Delta_V_step = 10e-3, Delta_V_limit = 1700e-3

  print*, R_1, R_NJ, R_NJ**2 / R_1**2
  ...
  do while (Delta_V <= Delta_V_limit)
