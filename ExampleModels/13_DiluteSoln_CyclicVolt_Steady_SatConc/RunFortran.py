...
fortran_template = 'DiluteSoln_CyclicVolt_Steady.f95'
_temp_geo = 'fortran_temp_geo.f95'
_temp_diff = 'fortran_temp_diff.f95'

dir_geo = ''
dir_diff = ''

diffusion_coef = ['Constant', 'Variable'] #'Constant',
Geometry = ['Rectangular', 'Spherical']
...
for geo in Geometry:
    ...
    if not(os.path.exists(dir_geo)):   # if folder does not exist, create it
        os.makedirs(dir_geo)

    sed('__GEOMETRY__', geo, fortran_template, _temp_geo)

    for diff_coef in diffusion_coef:
        os.chdir(cwd)

        dir_diff = dir_geo + '{}/'.format(diff_coef)

        if not(os.path.exists(dir_diff)):   # if folder does not exist, create it
            os.makedirs(dir_diff)

        sed('__diff_coef__', diff_coef, _temp_geo, _temp_diff)
