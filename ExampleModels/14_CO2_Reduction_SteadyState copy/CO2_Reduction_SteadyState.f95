...
module user_input
  ...
  integer, parameter :: NJ = 52                ! Number of mesh points
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then ! write headers
      ...
      write(*, data_fmt) Converge_BOOL,    c_1, c_NJ
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    BC_WEST_(1) = flux_temp(1) - 1e-5*cbulk/xmax*2.0
    ! BC_WEST_(1) = c_vars_dual(1) - 1.0
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = flux_temp(1) - 1e-5*cbulk/xmax*2.0

    ! BC_EAST_(1) = c_vars_dual(1) - 0.0
