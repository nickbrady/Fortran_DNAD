...
module user_input
  ...
  real               :: time                             ! [s]
  real, parameter    :: xmax = 23.2e-4 *2                      ! 100 um
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      ...
      xi       = zeta * (aa * viscosity / 3.0 / diff_coef)**(1./3.)
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'Conc',   'Velocity', 'Xi', 'Zeta'
      write(56, header_fmt) 'hours', 'cm',      'mol/cm3', 'cm/s'   , ''  , ''
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
      ...
      xi       = zeta * (aa * viscosity / 3.0 / diff_coef)**(1./3.)
                                                    !
      write(56, data_fmt) t_write, xx(j), conc, vel
