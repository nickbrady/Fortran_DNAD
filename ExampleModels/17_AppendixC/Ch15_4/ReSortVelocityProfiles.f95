...
module user_input
  ...
  real, parameter :: F_init = 0.0
  real, parameter :: G_init = 0.0
  real, parameter :: H_init = 0.0
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time', 'Position', 'F', 'G', 'H'
      write(*, header_fmt) 'hours', 'ζ',      '' , '' , ''
    end if

    do j = 1, NJ                                    !
      write(*, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j)
    end do
  ...
  subroutine write_positional_information_to_file(it)
    ...
    open(56, file = 'Velocity_Position.txt', status = 'unknown')

    if (it == 0) then
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'F', 'G', 'H'
      write(56, header_fmt) 'hours', 'ζ',      '' , '' , ''
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
                                                    !
      write(56, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j)
...
module GOV_EQNS
  ...
  type(dual) :: F_var, G_var, H_var
  type(dual) :: dFdx, dGdx, dHdx
  type(dual) :: d2Fdx2, d2Gdx2, d2Hdx2
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    F_var    = c_vars_dual(1)
    dFdx     = dcdx_vars_dual(1)
    d2Fdx2   = d2cdx2_vars_dual(1)

    G_var    = c_vars_dual(2)
    dGdx     = dcdx_vars_dual(2)
    d2Gdx2   = d2cdx2_vars_dual(2)

    H_var    = c_vars_dual(3)
    dHdx     = dcdx_vars_dual(3)
    d2Hdx2   = d2cdx2_vars_dual(3)
                                                                  ! Equations 15.26
    GOV_(1)     = 2*F_var + dHdx                                  ! 2F + H' = 0
    GOV_(2)     = F_var**2 - G_var**2 + H_var*dFdx - d2Fdx2       ! F^2 - G^2 + HF' = F''
    GOV_(3)     = 2*F_var*G_var + H_var*dGdx - d2Gdx2             ! 2FG + HG' = G''
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    dHdx     = dcdx_vars_dual(3)
                                        ! Equations 15.27
    BC_WEST_(1) = F_var - 0.0           ! F = 0.0
    BC_WEST_(2) = H_var - 0.0           ! H = 0.0
    BC_WEST_(3) = G_var - 1.0           ! G = 1.0
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    dHdx     = dcdx_vars_dual(3)
                                          ! Equations 15.27
    BC_EAST_(1) = F_var - 0.0             ! F = 0.0
    BC_EAST_(2) = G_var - 0.0             ! G = 0.0
    BC_EAST_(3) = 2*F_var + dHdx          !
