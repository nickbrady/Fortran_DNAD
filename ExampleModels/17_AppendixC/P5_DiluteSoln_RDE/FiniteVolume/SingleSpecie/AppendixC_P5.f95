...
module user_input
  ...
  real               :: time                             ! [s]
  real, parameter    :: xmax = 23.2e-4                      ! 100 um
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual, position)  result(Flux_)
    ...
    conc     = c_vars_dual(1)
    dcdx     = dcdx_vars_dual(1)

    vel      = velocity(position)

    Flux_(1) = -diff_coef * dcdx + vel * conc
  ...
  function RXN(c_vars_dual, position)                   result(Rxn_)
    ...
    Rxn_ = 0.0
    Rxn_(1) = +c_vars_dual(1) * vel * 2 / position  ! dv/dx = 2*vel/x
                                                    ! because vel = x^2
  end function RXN
