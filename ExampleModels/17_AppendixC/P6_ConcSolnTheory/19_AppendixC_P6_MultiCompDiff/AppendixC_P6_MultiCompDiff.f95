...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Iter', 'Position', 'C_O2', 'C_OH',  'C_Na',  'C_Cl',  'Phi',   'C_H2O'
      write(*, header_fmt) '#',    'η',       'mol/L', 'mol/L', 'mol/L', 'mol/L', 'Volts', 'mol/L'
    end if

    do j = 1, NJ                                    !
      c_H2O   = c_total_bulk*1000.*cprev(6,j)
      c_OH    = c_total_bulk*1000.*cprev(7,j)
      c_Cl    = c_total_bulk*1000.*cprev(8,j)
      c_Na    = c_total_bulk*1000.*cprev(9,j)
      c_O2    = c_total_bulk*1000.*cprev(10,j)
      Phi     = cprev(11,j) * Rigc*Temp/Fconst
                                                    !
      write(*, data_fmt) it, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, c_H2O
    end do
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Iter', 'Position', 'C_O2', 'C_OH',  'C_Na',  'C_Cl',  'Phi',   'C_H2O'
    ...
    do j = 1, NJ
      ...
      Phi     = cprev(11,j) * Rigc*Temp/Fconst
                                                    !
      write(56, data_fmt) it, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, c_H2O
    end do
...
module GOV_EQNS
  ...
  type(dual) :: d2cdx2_O2, d2cdx2_OH, d2cdx2_Na, d2cdx2_Cl, d2cdx2_H2O
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    ! Flux_i = N_i / c_total
    Flux_H2O    = c_vars_dual(1)
    dFlux_H2O_dx= dcdx_vars_dual(1)

    Flux_OH     = c_vars_dual(2)
    dFlux_OH_dx = dcdx_vars_dual(2)

    Flux_Cl     = c_vars_dual(3)
    dFlux_Cl_dx = dcdx_vars_dual(3)

    Flux_Na     = c_vars_dual(4)
    dFlux_Na_dx = dcdx_vars_dual(4)

    Flux_O2     = c_vars_dual(5)
    dFlux_O2_dx = dcdx_vars_dual(5)

    x_H2O       = c_vars_dual(6)
    dcdx_H2O    = dcdx_vars_dual(6)

    x_OH        = c_vars_dual(7)
    dcdx_OH     = dcdx_vars_dual(7)

    x_Cl        = c_vars_dual(8)
    dcdx_Cl     = dcdx_vars_dual(8)

    x_Na        = c_vars_dual(9)
    dcdx_Na     = dcdx_vars_dual(9)

    x_O2        = c_vars_dual(10)
    dcdx_O2     = dcdx_vars_dual(10)

    Phi         = c_vars_dual(11)
    dPhidx      = dcdx_vars_dual(11)

    GOV_(1)     = 2*position * dcdx_H2O - dFlux_H2O_dx
    GOV_(2)     = 2*position * dcdx_OH  - dFlux_OH_dx
    GOV_(3)     = 2*position * dcdx_Cl  - dFlux_Cl_dx
    GOV_(4)     = 2*position * dcdx_Na  - dFlux_Na_dx
    GOV_(5)     = 2*position * dcdx_O2  - dFlux_O2_dx

    GOV_(6)     = dcdx_H2O + z_H2O*x_H2O*dPhidx - &
                & D_O2*( 1/D_OH*(x_H2O*Flux_OH - x_OH*Flux_H2O) + 1/D_Cl*(x_H2O*Flux_Cl - x_Cl*Flux_H2O) + &
                &        1/D_Na*(x_H2O*Flux_Na - x_Na*Flux_H2O) + 1/D_O2*(x_H2O*Flux_O2 - x_O2*Flux_H2O) )
    GOV_(7)     = dcdx_OH + z_OH*x_OH*dPhidx - D_O2/D_OH*(x_OH*Flux_H2O - x_H2O*Flux_OH)
    GOV_(8)     = dcdx_Cl + z_Cl*x_Cl*dPhidx - D_O2/D_Cl*(x_Cl*Flux_H2O - x_H2O*Flux_Cl)
    GOV_(9)     = dcdx_Na + z_Na*x_Na*dPhidx - D_O2/D_Na*(x_Na*Flux_H2O - x_H2O*Flux_Na)

    Gov_(10)    = x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0
    GOV_(11)    = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O


    ! c_total     = c_total_bulk

    ! GOV_(5)     = 2*position * dcdx_O2  - dFlux_O2_dx
    ! GOV_(6)     = 2*position * dcdx_OH  - dFlux_OH_dx
    ! GOV_(7)     = 2*position * dcdx_Na  - dFlux_Na_dx
    ! GOV_(8)     = 2*position * dcdx_Cl  - dFlux_Cl_dx
    ! GOV_(9)     = 2*position * dcdx_H2O - dFlux_H2O_dx
    !
    ! GOV_(1)     = dcdx_O2 + z_O2*x_O2*Fconst/(Rigc*Temp)*dPhidx - D_O2/D_O2*(x_O2*Flux_H2O - x_H2O*Flux_O2)
    ! GOV_(2)     = dcdx_OH + z_OH*x_OH*Fconst/(Rigc*Temp)*dPhidx - D_O2/D_OH*(x_OH*Flux_H2O - x_H2O*Flux_OH)
    ! GOV_(3)     = dcdx_Na + z_Na*x_Na*Fconst/(Rigc*Temp)*dPhidx - D_O2/D_Na*(x_Na*Flux_H2O - x_H2O*Flux_Na)
    ! GOV_(4)     = dcdx_Cl + z_Cl*x_Cl*Fconst/(Rigc*Temp)*dPhidx - D_O2/D_Cl*(x_Cl*Flux_H2O - x_H2O*Flux_Cl)
    !
    !
    !
    ! GOV_(10)    = z_O2*c_O2 + z_OH*c_OH + z_Na*c_Na + z_Cl*c_Cl + z_H2O*c_H2O
    ! Gov_(11)    = (c_O2 + c_OH + c_Na + c_Cl + c_H2O)/c_total - 1.0

    ! GOV_(1)     = D_O2 * d2cdx2_vars_dual(1) - 1.0e-5/1e-3*c_O2**2
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    BC_WEST_(1) = Flux_H2O - s_H2O/s_O2 * Flux_O2
    BC_WEST_(2) = Flux_OH  - s_OH/s_O2 * Flux_O2
    BC_WEST_(3) = Flux_Cl  - s_Cl/s_O2 * Flux_O2
    BC_WEST_(4) = Flux_Na  - s_Na/s_O2 * Flux_O2
    BC_WEST_(5) = x_O2 - 0.0

    BC_WEST_(6) = dcdx_H2O + z_H2O*x_H2O*dPhidx - &
                & D_O2*( 1/D_OH*(x_H2O*Flux_OH - x_OH*Flux_H2O) + 1/D_Cl*(x_H2O*Flux_Cl - x_Cl*Flux_H2O) + &
                &        1/D_Na*(x_H2O*Flux_Na - x_Na*Flux_H2O) + 1/D_O2*(x_H2O*Flux_O2 - x_O2*Flux_H2O) )
    BC_WEST_(7) = dcdx_OH + z_OH*x_OH*dPhidx - D_O2/D_OH*(x_OH*Flux_H2O - x_H2O*Flux_OH)
    BC_WEST_(8) = dcdx_Cl + z_Cl*x_Cl*dPhidx - D_O2/D_Cl*(x_Cl*Flux_H2O - x_H2O*Flux_Cl)
    BC_WEST_(9) = dcdx_Na + z_Na*x_Na*dPhidx - D_O2/D_Na*(x_Na*Flux_H2O - x_H2O*Flux_Na)

    BC_WEST_(10)= x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0
    BC_WEST_(11)= z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    BC_EAST_(1) = 2*position * dcdx_H2O - dFlux_H2O_dx
    BC_EAST_(2) = 2*position * dcdx_OH  - dFlux_OH_dx
    BC_EAST_(3) = 2*position * dcdx_Cl  - dFlux_Cl_dx
    BC_EAST_(4) = 2*position * dcdx_Na  - dFlux_Na_dx
    BC_EAST_(5) = 2*position * dcdx_O2  - dFlux_O2_dx

    BC_EAST_(6) = x_Cl - cbulk_Cl / c_total_bulk
    BC_EAST_(7) = x_Na - cbulk_Na / c_total_bulk
    BC_EAST_(8) = x_O2 - cbulk_O2 / c_total_bulk
    BC_EAST_(9) = Phi - 0.0

    BC_EAST_(10)= x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0
    BC_EAST_(11)= z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O
...
subroutine initial_condition()
  ...
  cprev(1:5,:)  = 0.0                   ! N_i = 0.0
  cprev(7,:)    = cbulk_OH / c_total_bulk
  cprev(8,:)    = cbulk_Cl / c_total_bulk
  cprev(9,:)    = cbulk_Na / c_total_bulk
  cprev(10,:)   = cbulk_O2 / c_total_bulk * xx/xmax
  cprev(6,:)    = 1.0
  do i = 7, 10
    cprev(6,:)  = cprev(6,:) - cprev(i,:)   ! x_H2O = 1.0 - Σx_i
  end do
  cprev(11,:)   = 0.0                   ! Phi = 0.0
