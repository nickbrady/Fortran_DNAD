...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    GOV_(5)    = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    Flux_O2     = -dcdx_O2 - z_O2*x_O2*Fconst/(Rigc*Temp)*dPhidx
    Flux_OH     = D_OH/D_O2*(-dcdx_OH - z_OH*x_OH*Fconst/(Rigc*Temp)*dPhidx)
    Flux_Cl     = D_Cl/D_O2*(-dcdx_Cl - z_Cl*x_Cl*Fconst/(Rigc*Temp)*dPhidx)
    Flux_Na     = D_Na/D_O2*(-dcdx_Na - z_Na*x_Na*Fconst/(Rigc*Temp)*dPhidx)
