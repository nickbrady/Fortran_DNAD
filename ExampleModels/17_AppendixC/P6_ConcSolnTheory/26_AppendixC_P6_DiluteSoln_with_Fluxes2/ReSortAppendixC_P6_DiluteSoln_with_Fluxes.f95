...
contains
  ...
  subroutine write_to_screen(it)
    ...
    ! if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Iter', 'Position', 'C_O2', 'C_OH',  'C_Na',  'C_Cl',  'Phi', 'Flux_OH', 'Flux_Cl', &
      & 'Flux_Na', 'Flux_O2'
    ...
    do j = 1, NJ                                    !
      ...
      c_O2    = c_total_bulk*1000.*cprev(4,j)
      Phi     = cprev(5,j)
                                                    !
      write(*, data_fmt) it, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, cprev(6,j), cprev(7,j), cprev(8,j), cprev(9,j)
    end do
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Iter', 'Position', 'C_O2', 'C_OH',  'C_Na',  'C_Cl',  'Phi', 'Flux_OH', 'Flux_Cl', &
      & 'Flux_Na', 'Flux_O2'
    ...
    do j = 1, NJ
      ...
      Phi     = cprev(5,j)
                                                    !
      write(56, data_fmt) it, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, cprev(6,j), cprev(7,j), cprev(8,j), cprev(9,j)
    end do
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    x_O2        = c_vars_dual(4)
    dcdx_O2     = dcdx_vars_dual(4)
    d2cdx2_O2   = d2cdx2_vars_dual(4)
    ...
    Flux_O2     = c_vars_dual(9)
    dFlux_O2_dx = dcdx_vars_dual(9)
    ...
    GOV_(4)     = 2*position * dcdx_O2 - dFlux_O2_dx
    ...
    GOV_(9)    = Flux_O2 + D_O2/D_O2 * (dcdx_O2 + z_O2*x_O2 * Fconst/(Rigc*Temp)*dPhidx)
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    BC_WEST_(9) = Flux_O2 + D_O2/D_O2 * (dcdx_O2 + z_O2*x_O2 * Fconst/(Rigc*Temp)*dPhidx)
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    BC_EAST_(4) = x_O2 - cbulk_O2 / c_total_bulk
    BC_EAST_(5) = Phi - 0.0
    ...
    BC_EAST_(9) = 2*position * dcdx_O2 - dFlux_O2_dx

    BC_EAST_(6) = 2*position * dcdx_OH - 0.0
    BC_EAST_(7) = 2*position * dcdx_Cl - 0.0
    BC_EAST_(8) = 2*position * dcdx_Na - 0.0
    BC_EAST_(9) = 2*position * dcdx_O2 - 0.0
...
program unsteady
  ...
  do while ((convergence_error > tol).AND.(it <= maxIterations)) !it = 1, 360*2 !Numbertimesteps
      ...
      ! **********************************************************************
        ...
        cprev = cprev + 0.1*delC      ! Update the dependent variables
        time  = time + delT
        ! ********************************************************************
        ! call write_condition(it)
...
subroutine initial_condition()
  ...
  cprev(4,:)   = cbulk_O2 / c_total_bulk! * xx/xmax
  cprev(5,:)   = 0.0                    ! Phi = 0.0
...
subroutine auto_fill(j)
  ...
  !-----------------------------------------------------------------------------
    ...
    GOV_dual = GOV_RHS(cj_dual, dcdx_dual, d2cdx2_dual, xx(j))    ! West Side Flux
