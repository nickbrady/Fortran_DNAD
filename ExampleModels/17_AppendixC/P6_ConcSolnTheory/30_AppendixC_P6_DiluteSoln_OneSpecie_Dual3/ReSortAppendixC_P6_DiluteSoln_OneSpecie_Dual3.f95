...
module user_input
  ...
  integer, parameter :: maxIterations = 20!1e6
  real               :: delT = 1.0e-1                       ! size of timestep [s]
  real               :: time                             ! [s]
  real, parameter    :: xmax = 6.0                      ! 100 um
  ...
  ! real            :: viscosity = 1e-3 * 10 / density ! cm2 / s
  ! real            :: omega = 500*2*PI/60             ! angular velocity of RDE (radians/second)

  real, parameter :: cbulk_O2 = 2e-7
  real, parameter :: cbulk_OH = 0.0
  real, parameter :: cbulk_Na = 1e-3
  real, parameter :: cbulk_Cl = 1e-3
  real, parameter :: c_total_bulk = 55.5e-3
  real, parameter :: cbulk_H2O = c_total_bulk - cbulk_O2 - cbulk_OH - cbulk_Na - cbulk_Cl

  real, parameter :: D_O2 = 2e-5
  real, parameter :: D_OH = 5.26e-5
  real, parameter :: D_Na = 1.334e-5
  real, parameter :: D_Cl = 2.032e-5

  real, parameter :: z_O2 = 0.0
  real, parameter :: z_OH = -1.0
  ...
  real, parameter :: s_Na = 0.0
  real, parameter :: s_Cl = 0.0
  real, parameter :: s_H2O = 0.0

  ! real, parameter :: u_OH = D_OH / (Rigc * Temp)
  ! real, parameter :: u_O2 = D_O2 / (Rigc * Temp)
  ! real, parameter :: u_Na = D_Na / (Rigc * Temp)
  ! real, parameter :: u_Cl = D_Cl / (Rigc * Temp)

  character(len=65) :: geometry = 'Rectangular'   ! system Geometry: Rectangular, Cylindrical, Spherical
  character(len=65) :: matrix_fmt = '(20(ES15.8,2X))'
...
module write_data_mod
  ...
  character(len=65) :: header_fmt = '(1(A5,2X), 1(A12,2X),   20(A15,2X)) '
  character(len=65) :: data_fmt   = '(1(I5,2X), 1(F12.5,2X), 20(ES15.8,2X))'

  real :: t_write

  real :: conc_1, conc_NJ, vel_1, vel_NJ
  real :: c_O2, c_OH, c_Na, c_Cl, c_H2O, Phi
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    ! if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Iter', 'Position', 'C_O2', 'Flux_O2',  'C_Na',  'C_Cl',  'Phi', 'Flux_OH', 'Flux_Cl', &
      & 'Flux_Na', 'Flux_O2'
      write(*, header_fmt) '#',    'η',       'mol/L', 'mol/L', 'mol/L', 'mol/L', 'Volts', 'mol/L', '', '', '', ''
    ! end if

    do j = 1, NJ                                    !
      ! c_OH    = c_total_bulk*1000.*cprev(1,j)
      ! c_Cl    = c_total_bulk*1000.*cprev(2,j)
      ! c_Na    = c_total_bulk*1000.*cprev(3,j)
      c_O2    = c_total_bulk*1000.*cprev(1,j)
      ! Phi     = cprev(5,j)
                                                    !
      write(*, data_fmt) it, xx(j), c_O2, cprev(2,j)!, c_OH, c_Na, c_Cl, Phi, cprev(6,j), cprev(7,j), cprev(8,j), cprev(9,j)
    end do
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Iter', 'Position', 'C_O2', 'Flux_O2',  'C_Na',  'C_Cl',  'Phi', 'Flux_OH', 'Flux_Cl', &
      & 'Flux_Na', 'Flux_O2'
    ...
    do j = 1, NJ
      ...
      ! Phi     = cprev(5,j)
                                                    !
      write(56, data_fmt) it, xx(j), c_O2, cprev(2,j)!, c_OH, c_Na, c_Cl, Phi, cprev(6,j), cprev(7,j), cprev(8,j), cprev(9,j)
    end do
...
module GOV_EQNS
  ...
  type(dual) :: x_O2, x_OH, x_Na, x_Cl, x_H2O, Phi
  type(dual) :: c_O2, c_OH, c_Na, c_Cl, c_H2O
  type(dual) :: dcdx_O2, dcdx_OH, dcdx_Na, dcdx_Cl, dcdx_H2O, dPhidx
  type(dual) :: d2cdx2_O2, d2cdx2_OH, d2cdx2_Na, d2cdx2_Cl, d2cdx2_H2O, d2Phidx2
  type(dual) :: Flux_O2, Flux_OH, Flux_Na, Flux_Cl, Flux_H2O
  type(dual) :: dFlux_O2_dx, dFlux_OH_dx, dFlux_Na_dx, dFlux_Cl_dx, dFlux_H2O_dx
  type(dual) :: Flux_, dFlux__dx
  real       :: vel, position
  real       :: c_total
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    ! x_OH        = c_vars_dual(1)
    ! dcdx_OH     = dcdx_vars_dual(1)
    ! d2cdx2_OH   = d2cdx2_vars_dual(1)
    !
    ! x_Cl        = c_vars_dual(2)
    ...
    !
    ! x_Na        = c_vars_dual(3)
    ! dcdx_Na     = dcdx_vars_dual(3)
    ! d2cdx2_Na   = d2cdx2_vars_dual(3)

    x_O2        = c_vars_dual(1)
    dcdx_O2     = dcdx_vars_dual(1)
    d2cdx2_O2   = d2cdx2_vars_dual(1)

    ! Phi         = c_vars_dual(5)
    ! dPhidx      = dcdx_vars_dual(5)
    ! d2Phidx2    = d2cdx2_vars_dual(5)
    !
    ! Flux_OH     = c_vars_dual(6)
    ! dFlux_OH_dx = dcdx_vars_dual(6)
    !
    ! Flux_Cl     = c_vars_dual(7)
    ! dFlux_Cl_dx = dcdx_vars_dual(7)
    !
    ! Flux_Na     = c_vars_dual(8)
    ! dFlux_Na_dx = dcdx_vars_dual(8)
    !
    ! Flux_O2     = c_vars_dual(2)
    ! dFlux_O2_dx = dcdx_vars_dual(2)

    ! GOV_(1)     = 2*position * dcdx_OH - dFlux_OH_dx    ! 2η dx/dη = dN/dη
    ! GOV_(2)     = 2*position * dcdx_Cl - dFlux_Cl_dx
    ! GOV_(3)     = 2*position * dcdx_Na - dFlux_Na_dx
    ! GOV_(1)     = 2*position * dcdx_O2 - dFlux_O2_dx

    ! GOV_(2)     = z_Cl*dFlux_Cl_dx + z_O2*dFlux_O2_dx + z_OH*dFlux_OH_dx + z_Na*dFlux_Na_dx

    ! GOV_(1)     = 2*position * dcdx_OH + &
    !               & D_OH/D_O2 * (d2cdx2_OH + z_OH*dcdx_OH * Fconst/(Rigc*Temp)*dPhidx + z_OH*x_OH * Fconst/(Rigc*Temp)*d2Phidx2)
    ! GOV_(2)     = 2*position * dcdx_Cl + &
    !               & D_Cl/D_O2 * (d2cdx2_Cl + z_Cl*dcdx_Cl * Fconst/(Rigc*Temp)*dPhidx + z_Cl*x_Cl * Fconst/(Rigc*Temp)*d2Phidx2)
    ! GOV_(3)     = 2*position * dcdx_Na + &
    !               & D_Na/D_O2 * (d2cdx2_Na + z_Na*dcdx_Na * Fconst/(Rigc*Temp)*dPhidx + z_Na*x_Na * Fconst/(Rigc*Temp)*d2Phidx2)
    GOV_(1)     = 2*position * dcdx_O2 + &
                  & D_O2/D_O2 * (d2cdx2_O2)! + z_O2*dcdx_O2 * Fconst/(Rigc*Temp)*dPhidx + z_O2*x_O2 * Fconst/(Rigc*Temp)*d2Phidx2)

    ! GOV_(5)    = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl - 0.0    ! Σ(z_i x_i) = 0.0
    !                                                                     ! N_i = -D_i/D_R * (dx/dη + z_i x_i F/RT dΦ/dη)
    ! GOV_(6)    = Flux_OH + D_OH/D_O2 * (dcdx_OH + z_OH*x_OH * Fconst/(Rigc*Temp)*dPhidx)
    ! GOV_(7)    = Flux_Cl + D_Cl/D_O2 * (dcdx_Cl + z_Cl*x_Cl * Fconst/(Rigc*Temp)*dPhidx)
    ! GOV_(8)    = Flux_Na + D_Na/D_O2 * (dcdx_Na + z_Na*x_Na * Fconst/(Rigc*Temp)*dPhidx)
    ! GOV_(2)    = Flux_O2 + D_O2/D_O2 * (dcdx_O2)! + z_O2*x_O2 * Fconst/(Rigc*Temp)*dPhidx)

    ! Flux_     = -D_Cl/D_O2 * (dcdx_Cl + z_Cl*x_Cl * Fconst/(Rigc*Temp)*dPhidx)
    ! dFlux__dx = -D_Cl/D_O2 * (d2cdx2_Cl + z_Cl*dcdx_Cl * Fconst/(Rigc*Temp)*dPhidx + z_Cl*x_Cl * Fconst/(Rigc*Temp)*d2Phidx2)
    ! print*, 'Flux_Cl'
    ! print*, dFlux__dx%x, Flux_%x
    ! print*, dFlux_Cl_dx%x, Flux_Cl%x
    ! print*, 2*position * dcdx_Cl%x
    ! print*
    !
    ! Flux_     = -D_Na/D_O2 * (dcdx_Na + z_Na*x_Na * Fconst/(Rigc*Temp)*dPhidx)
    ! dFlux__dx = -D_Na/D_O2 * (d2cdx2_Na + z_Na*dcdx_Na * Fconst/(Rigc*Temp)*dPhidx + z_Na*x_Na * Fconst/(Rigc*Temp)*d2Phidx2)
    ! print*, 'Flux_Na'
    ! print*, dFlux__dx%x, Flux_%x
    ! print*, dFlux_Na_dx%x, Flux_Na%x
    ! print*, 2*position * dcdx_Na%x
    ! print*
    !
    ! Flux_     = -D_OH/D_O2 * (dcdx_OH + z_OH*x_OH * Fconst/(Rigc*Temp)*dPhidx)
    ! dFlux__dx = -D_OH/D_O2 * (d2cdx2_OH + z_OH*dcdx_OH * Fconst/(Rigc*Temp)*dPhidx + z_OH*x_OH * Fconst/(Rigc*Temp)*d2Phidx2)
    ! print*, 'Flux_OH'
    ! print*, dFlux__dx%x, Flux_%x
    ! print*, dFlux_OH_dx%x, Flux_OH%x
    ! print*, 2*position * dcdx_OH%x
    ! print*

    ! Flux_     = -D_O2/D_O2 * (dcdx_O2 + z_O2*x_O2 * Fconst/(Rigc*Temp)*dPhidx)
    ! dFlux__dx = -D_O2/D_O2 * (d2cdx2_O2 + z_O2*dcdx_O2 * Fconst/(Rigc*Temp)*dPhidx + z_O2*x_O2 * Fconst/(Rigc*Temp)*d2Phidx2)
    ! print*, 'Flux_O2'
    ! print*, dFlux__dx%x, Flux_%x
    ! print*, dFlux_O2_dx%x, Flux_O2%x
    ! print*, 2*position * dcdx_O2%x - dFlux__dx%x
    ! print*, 'dcdx_O2', dcdx_O2%x
  ...
  function ACCUM(c_vars_dual, position)                           result(Accum_)
    ...
    ! Accum_(2) = c_vars_dual(2)/delT
    ! Accum_(3) = c_vars_dual(3)/delT
    ! Accum_(4) = c_vars_dual(4)/delT
    ! Accum_(5) = c_vars_dual(5)/delT
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    real, intent(in)                       :: position
    type(dual) :: Flux_O2_direct, Flux_OH_direct, Flux_Cl_direct, Flux_Na_direct
    ...
    ! dFlux_O2_dx = dcdx_vars_dual(2)
    !
    ! Flux_O2_direct     = -dcdx_O2 - z_O2*x_O2*Fconst/(Rigc*Temp)*dPhidx
    ! Flux_OH_direct     = D_OH/D_O2*(-dcdx_OH - z_OH*x_OH*Fconst/(Rigc*Temp)*dPhidx)
    ! Flux_Cl_direct     = D_Cl/D_O2*(-dcdx_Cl - z_Cl*x_Cl*Fconst/(Rigc*Temp)*dPhidx)
    ! Flux_Na_direct     = D_Na/D_O2*(-dcdx_Na - z_Na*x_Na*Fconst/(Rigc*Temp)*dPhidx)
    !
    ! BC_WEST_(1) = Flux_OH - s_OH/s_O2 * Flux_O2
    ! BC_WEST_(2) = Flux_Cl - s_Cl/s_O2 * Flux_O2
    ! BC_WEST_(3) = Flux_Na - s_Na/s_O2 * Flux_O2
    BC_WEST_(1) = x_O2 - 0.0
    ! BC_WEST_(1) = Flux_OH - s_OH/s_O2 * Flux_O2
    ! BC_WEST_(2) = Flux_Cl - s_Cl/s_O2 * Flux_O2.
    ! BC_WEST_(3) = Flux_Na - s_Na/s_O2 * Flux_O2

    ! BC_WEST_(5) = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O
    !
    ! BC_WEST_(6) = Flux_OH + D_OH/D_O2 * (dcdx_OH + z_OH*x_OH * Fconst/(Rigc*Temp)*dPhidx)
    ! BC_WEST_(7) = Flux_Cl + D_Cl/D_O2 * (dcdx_Cl + z_Cl*x_Cl * Fconst/(Rigc*Temp)*dPhidx)
    ! BC_WEST_(8) = Flux_Na + D_Na/D_O2 * (dcdx_Na + z_Na*x_Na * Fconst/(Rigc*Temp)*dPhidx)
    ! BC_WEST_(2) = Flux_O2 + D_O2/D_O2 * (dcdx_O2)! + z_O2*x_O2 * Fconst/(Rigc*Temp)*dPhidx)
    ! BC_WEST_(2) = dFlux_O2_dx - 0.0

    print*,
    print*, Flux_OH_direct%x, Flux_Cl_direct%x, Flux_Na_direct%x, Flux_O2_direct%x
    print*, Flux_OH%x,        Flux_Cl%x,        Flux_Na%x,        Flux_O2%x
    print*,
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    ! dFlux_O2_dx = dcdx_vars_dual(2)
    !
    ! BC_EAST_(1) = x_OH - cbulk_OH / c_total_bulk
    ! BC_EAST_(2) = x_Cl - cbulk_Cl / c_total_bulk
    ! BC_EAST_(3) = x_Na - cbulk_Na / c_total_bulk
    BC_EAST_(1) = x_O2 - cbulk_O2 / c_total_bulk
    ! BC_EAST_(5) = Phi - 0.0
    !
    ! BC_EAST_(6) = 2*position * dcdx_OH - dFlux_OH_dx
    ! BC_EAST_(7) = 2*position * dcdx_Cl - dFlux_Cl_dx
    ! BC_EAST_(8) = 2*position * dcdx_Na - dFlux_Na_dx
    ! BC_EAST_(1) = 2*position * dcdx_O2 - dFlux_O2_dx
    ! BC_EAST_(1) = Flux_O2 + D_O2/D_O2 * (dcdx_O2)
    ! BC_EAST_(2) = Flux_O2 - 0.0
    ! BC_EAST_(2) = dcdx_O2 - 0.0
...
subroutine initial_condition()
  ...
  ! cprev(1,:)   = cbulk_OH / c_total_bulk
  ! cprev(2,:)   = cbulk_Cl / c_total_bulk
  ! cprev(3,:)   = cbulk_Na / c_total_bulk
  cprev(1,:)   = cbulk_O2 / c_total_bulk! * xx/xmax
  ! cprev(5,:)   = 0.0                    ! Phi = 0.0
  ! cprev(6:9,:) = 0.0                    ! Fluxes
  ! cprev(2,:)  = 0.0
...
subroutine auto_fill(j)
  ...
  real, dimension(N) :: dcdx, d2cdx2, dcdx_back, dcdx_forw
  ...
  if (j == 1) then                              ! West Side Boundary Conditions
    ...
    smP(1,1) = 0.0
    smE(1,1) = 1.0
    smF(1)   = 0.0 - smE(1,1)*cprev(1,j)

    ! N  = -dx/dη - forward difference
    smP(2,1) = 1.0
    smE(2,2) = 1.0
    smF(2)   = 0.0 - smP(2,1)*dcdx_forw(1) - smE(2,2)*cprev(2,j)

    ! dN = 2η dx/dη
    ! dN = 0.0
    ! smP(2,2) = 1.0
    ! smE(2,:) = 0.0
    ! smF(2)   = 0.0 - smP(2,2)*dcdx(2)
    ...
    !
    ! do ic = 1,N
    !     smP(ic, :) = boundary_conditionE(ic)%dx(N+1:2*N)        ! smP * dcdx +
    !     smE(ic, :) = boundary_conditionE(ic)%dx(1:N)            ! smE * c +
    !
    !     smF(ic)   = -boundary_conditionE(ic)%x                  ! smF = 0.0
    ! end do

    ! x = x_bulk
    smP(1,1) = 0.0
    smE(1,1) = 1.0
    smF(1)   = cbulk_O2 / c_total_bulk - smE(1,1)*cprev(1,j)

    ! N  = -dx/dη
    ! smP(2,1) = 1.0
    ! smE(2,2) = 1.0
    ! smF(2)   = 0.0 - smP(2,1)*dcdx(1) - smE(2,2)*cprev(2,j)

    ! dN = 2η dx/dη - backward difference
    smP(2,1) = 2*xx(j)
    smP(2,2) = -1.0
    smE(2,:) = 0.0
    smF(2)   = 0.0 - smP(2,1)*dcdx_back(1) - smP(2,2)*dcdx_back(2)

    A = -smP                     ! cprev(:, j-1)
    B = smP + h*smE              ! cprev(:, j)
    D(1:N,1:N) = 0.0                    ! cprev(:, j+1)
    G = h*smG
  ...
  !-----------------------------------------------------------------------------
    ...
    ! smB(1,1) = 2*xx(j)
    ! smD(1,1) = 0.0
    ! smG(1)   = 0.0 - smA(1,1)*d2cdx2(1) - smB(1,1)*dcdx(1)
    ...
    smB(1,2) = -1.0
    smD(1,:) = 0.0
    smG(1)   = 0.0 - smB(1,1)*dcdx_back(1) - smB(1,2)*dcdx_back(2)

    A(1,:) = -smB(1,:)                     ! cprev(:, j-1)
    B(1,:) = smB(1,:) + h*smD(1,:)              ! cprev(:, j)
    D(1,1:N) = 0.0                    ! cprev(:, j+1)
    G(1) = h*smG(1)
    ...
    smB(2,1) = 1.0
    smD(2,2) = 1.0
    smG(2)   = 0.0 - smB(2,1)*dcdx_forw(1) - smD(2,2)*cprev(2,j)

    A(2,:) = 0.0                        ! cprev(:, j-1)
    B(2,:) = -smB(2,:) + h*smD(2,:)               ! cprev(:, j)
    D(2,1:N) = smB(2,:)                    ! cprev(:, j+1)
    G(2) = h*smG(2)
...
subroutine ABDGXY(j)
  ...
  ! The coefficents A, B, D, G, X, Y can be found in Newman appendix C.
      ...
      A = smA - h/2.0*smB                 ! cprev(:, j-1)
      B = -2.0*smA + h**2*smD             ! cprev(:, j)
      D(1:N,1:N) = smA + h/2.0*smB        ! cprev(:, j+1)
      G = h**2*smG

      ! ! dcdx_L  -->  (cprev(:,j) - cprev(:,j-1)) / h
      ! A = smA - h*smB                     ! cprev(:, j-1)
      ! B = -2.0*smA + h*smB + h**2*smD     ! cprev(:, j)
      ! D(1:N,1:N) = smA                    ! cprev(:, j+1)
      ! G = h**2*smG
      !
      ! if(j.eq.1) then
      !   B = h*smE - smP
      ...
      ! ! dcdx_R  -->  (cprev(:,j+1) - cprev(:,j)) / h
      ! A = smA                             ! cprev(:, j-1)
      ! B = -2.0*smA - h*smB + h**2*smD     ! cprev(:, j)
      ! D(1:N,1:N) = smA + h*smB            ! cprev(:, j+1)
      ! G = h**2*smG
      !
      ! if(j.eq.1) then
      !   B = h*smE - smP
