...
module user_input
  ...
  real               :: delT = 1.0e6                       ! size of timestep [s]
  real               :: time                             ! [s]
  real, parameter    :: xmax = 2.0                      ! 100 um
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      ...
      write(*, header_fmt) 'hours', 'mol/cm3', 'mol/cm3', 'cm/s' , 'cm/s'  , ''     , ''
    end if

      conc_1   = cprev(1,1)
      conc_NJ  = cprev(1,NJ)
      ! vel_1    = velocity(xx(1))
      ! vel_NJ   = velocity(xx(NJ))
      ! zeta     = xx(NJ) * (omega/viscosity)**0.5
      ! xi       = zeta * (aa * viscosity / 3.0 / D_O2)**(1./3.)


      ! print*,
      write(*, data_fmt) t_write, conc_1, conc_NJ
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'C_O2', 'C_OH',    'C_Na',    'C_Cl',    'Phi',   'Velocity', 'Xi', 'Zeta'
      write(56, header_fmt) 'hours', 'cm',   'mol/cm3', 'mol/cm3', 'mol/cm3', 'mol/cm3', 'Volts', 'cm/s'    , ''  , ''
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
      ...
      ! Phi     = cprev(5,j)
      ! vel     = velocity(xx(j))
      ! zeta     = xx(j) * (omega/viscosity)**0.5
      ! xi       = zeta * (aa * viscosity / 3.0 / D_O2)**(1./3.)
                                                    !
      write(56, data_fmt) t_write, xx(j), c_O2!, c_OH, c_Na, c_Cl, Phi
...
module GOV_EQNS
  ...
  type(dual) :: c_O2, c_OH, c_Na, c_Cl, Phi
  type(dual) :: dcdx_O2, dcdx_OH, dcdx_Na, dcdx_Cl, dPhidx
  type(dual) :: d2cdx2_O2, d2cdx2_OH, d2cdx2_Na, d2cdx2_Cl, d2Phidx2
  real       :: vel, position
  real       :: c_total
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    GOV_(1)     = d2cdx2_O2 + 3 * position*dcdx_O2
    ...
    ! d2Phidx2    = d2cdx2_vars_dual(4)


    ! GOV_(1)     = D_O2/D_O2*d2cdx2_O2 + 3*position**2*dcdx_O2 + z_O2*u_O2*Fconst/D_O2*(c_O2*d2Phidx2 + dcdx_O2*dPhidx)
    ! GOV_(2)     = D_OH/D_O2*d2cdx2_OH + 3*position**2*dcdx_OH + z_OH*u_OH*Fconst/D_O2*(c_OH*d2Phidx2 + dcdx_OH*dPhidx)
    ! GOV_(3)     = D_Na/D_O2*d2cdx2_Na + 3*position**2*dcdx_Na + z_Na*u_Na*Fconst/D_O2*(c_Na*d2Phidx2 + dcdx_Na*dPhidx)
    ! GOV_(4)     = D_Cl/D_O2*d2cdx2_Cl + 3*position**2*dcdx_Cl + z_Cl*u_Cl*Fconst/D_O2*(c_Cl*d2Phidx2 + dcdx_Cl*dPhidx)
    !
    ! GOV_(5)     = z_O2*c_O2 + z_OH*c_OH + z_Na*c_Na + z_Cl*c_Cl
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    ! dPhidx      = dcdx_vars_dual(5)
    !
    ! flux_limiting = z_O2*u_O2*Fconst*c_O2*dPhidx + D_O2*dcdx_O2

    BC_WEST_(1) = c_O2 - 0.0
    ! BC_WEST_(2) = z_OH*u_OH*Fconst*c_OH*dPhidx + D_OH*dcdx_OH - s_OH/s_O2 * flux_limiting
    ! BC_WEST_(3) = z_Na*u_Na*Fconst*c_Na*dPhidx + D_Na*dcdx_Na - s_Na/s_O2 * flux_limiting
    ! BC_WEST_(4) = z_Cl*u_Cl*Fconst*c_Cl*dPhidx + D_Cl*dcdx_Cl - s_Cl/s_O2 * flux_limiting
    !
    ! BC_WEST_(5) = z_O2*c_O2 + z_OH*c_OH + z_Na*c_Na + z_Cl*c_Cl
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    BC_EAST_(1) = c_O2 - cbulk_O2
    ! BC_EAST_(2) = c_OH - cbulk_OH
    ! BC_EAST_(3) = c_Na - cbulk_Na
    ! BC_EAST_(4) = c_Cl - cbulk_Cl
    ! BC_EAST_(5) = Phi - 0.0
