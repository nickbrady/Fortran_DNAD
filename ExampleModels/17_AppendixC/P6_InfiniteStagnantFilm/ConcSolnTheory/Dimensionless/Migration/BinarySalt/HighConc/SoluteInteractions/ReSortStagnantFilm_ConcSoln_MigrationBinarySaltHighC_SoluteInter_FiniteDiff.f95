...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    GOV_(5)    = dcdx_Na + z_Na*Fconst*x_Na/(Rigc*Temp)*dPhidx &
              & - D_NaCl/D_Na*(x_Na * N_H2O - x_H2O * N_Na) - D_NaCl/D_NaCl*(x_Na * N_Cl - x_Cl * N_Na)
    GOV_(6)   = dcdx_Cl + z_Cl*Fconst*x_Cl/(Rigc*Temp)*dPhidx &
              & - D_NaCl/D_Cl*(x_Cl * N_H2O - x_H2O * N_Cl) - D_NaCl/D_NaCl*(x_Cl * N_Na - x_Na * N_Cl)
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    BC_WEST_(5)   =  dcdx_Na + z_Na*Fconst*x_Na/(Rigc*Temp)*dPhidx &
                    & - D_NaCl/D_Na*(x_Na * N_H2O - x_H2O * N_Na) - D_NaCl/D_NaCl*(x_Na * N_Cl - x_Cl * N_Na)
    BC_WEST_(6)   = dcdx_Cl + z_Cl*Fconst*x_Cl/(Rigc*Temp)*dPhidx &
                    & - D_NaCl/D_Cl*(x_Cl * N_H2O - x_H2O * N_Cl) - D_NaCl/D_NaCl*(x_Cl * N_Na - x_Na * N_Cl)
