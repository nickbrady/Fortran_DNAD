...
module user_input
  ...
  integer, parameter :: NJ = 151                          ! Number of mesh points
  ...
  real, parameter :: xbulk_Li = 0.1 !c_Li / c_total
  real, parameter :: xbulk_BMP = 0.5 - xbulk_Li!c_BMP / c_total
  real, parameter :: xbulk_TFSI = 1.0 - xbulk_Li - xbulk_BMP
  real  :: x_Li_electrode
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  'Position', 'x_Li', 'x_BMP', 'x_TFSI', 'Flux_Li', 'Flux_BMP', 'Flux_TFSI', 'Phi'
      write(*, header_fmt) 'hours', 'ξ'       , ''    , ''    , ''     , ''       , ''       , ''        , 'Volts'
    end if

    do j = 1, NJ                                    !
                                                          !
      write(*, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j), cprev(4,j), &
      &   cprev(5,j), cprev(6,j), cprev(7,j)
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time',  'Position', 'x_Li', 'x_BMP', 'x_TFSI', 'Flux_Li', 'Flux_BMP', 'Flux_TFSI', 'Phi'
    ...
    do j = 1, NJ                                    !
                                                          !
      write(56, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j), cprev(4,j), &
      &   cprev(5,j), cprev(6,j), cprev(7,j)
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    N_Li       = c_vars_dual(4)
    dNdx_Li    = dcdx_vars_dual(4)

    N_BMP       = c_vars_dual(5)
    dNdx_BMP    = dcdx_vars_dual(5)
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    ! O2 + 4e- + 2*H2O --> 4 OH-
    ! x_O2 = 0.0
    BC_WEST_(1) = x_Li - xbulk_Li *0.1
    BC_WEST_(2) = N_BMP - 0.0
    BC_WEST_(3) = N_TFSI - 0.0

    !             Σ x_i = 1
    BC_WEST_(4)    = x_Li + x_BMP + x_TFSI - 1.0

    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    BC_WEST_(5)   = dcdx_Li + z_Li*Fconst*x_Li/(Rigc*Temp)*dPhidx &
                    & - D_LiTFSI/D_LiTFSI*(x_Li * N_TFSI - x_TFSI * N_Li) - D_LiTFSI/D_LiBMP*(x_Li * N_BMP - x_BMP * N_Li)
    BC_WEST_(6)   = dcdx_BMP + z_BMP*Fconst*x_BMP/(Rigc*Temp)*dPhidx &
                    & - D_LiTFSI/D_BMPTFSI*(x_BMP * N_TFSI - x_TFSI * N_BMP) - D_LiTFSI/D_LiBMP*(x_BMP * N_Li - x_Li * N_BMP)

    !             Σ c_i z_i = 0 = Σ x_i z_i
    BC_WEST_(7)   = z_Li*x_Li + z_BMP*x_BMP + z_TFSI*x_TFSI - 0.0
    ! BC_WEST_(11) = Phi - 0.0
    ! print*, x_Li%x, xbulk_Li * 0.1, x_Li%x - xbulk_Li*0.1
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    BC_EAST_(3) = 2*position*dcdx_TFSI - dNdx_TFSI

    !             Σ x_i = 1
    BC_EAST_(4)    = x_Li + x_BMP + x_TFSI - 1.0

    ! x_i = x_i_bulk
    ! Φ = 0.0
    BC_EAST_(5)   = x_Li - xbulk_Li
    BC_EAST_(6)   = x_BMP - xbulk_BMP
    BC_EAST_(7)   = Phi - 0.0
