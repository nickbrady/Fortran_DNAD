...
module user_input
  ...
  ! real, parameter :: c_BMP = 7e-3
  ! real, parameter :: c_Li = c_BMP*1e-2
  ! real, parameter :: c_TFSI = c_Li + c_BMP
  ! real, parameter :: c_total = c_Li + c_BMP + c_TFSI
  ! real, parameter :: xbulk_O2 = 2e-7 / c_total
  ! real, parameter :: xbulk_OH = 0.0
  real, parameter :: xbulk_Li = 0.45 !c_Li / c_total
  real, parameter :: xbulk_BMP = 0.5 - xbulk_Li!c_BMP / c_total
  real, parameter :: xbulk_TFSI = 0.5!1.0 - xbulk_Li - xbulk_BMP
  real, parameter :: x_TFSI = 0.5
  real, parameter :: N_TFSI = 0.0

  ! real, parameter :: D_O2 = 2.00000e-5
  ! real, parameter :: D_OH = 5.26000e-5
  real, parameter :: D_LiTFSI = 1.33400e-5
  real, parameter :: D_BMPTFSI = 2.03200e-5
  real, parameter :: D_LiBMP  = 3e-5

  ! real, parameter :: z_O2 =  0.0
  ! real, parameter :: z_OH = 0.0!-1.0
  real, parameter :: z_Li = +1.0
  real, parameter :: z_BMP = +1.0
  real, parameter :: z_TFSI = -1.0

  ! real, parameter :: s_O2 = +1.0
  ! real, parameter :: s_OH = -4.0
  ! real, parameter :: s_Li =  1.0
  ! real, parameter :: s_BMP =  0.0
  ! real, parameter :: s_TFSI = 0.0!+2.0
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  'Position', 'x_Li', 'x_BMP', 'Flux_Li', 'Flux_BMP', 'Phi'
      write(*, header_fmt) 'hours', 'ξ'       , ''    , ''     , ''       , ''        , ''
    end if

    do j = 1, NJ                                    !
                                                          !
      write(*, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j), cprev(4,j), cprev(5,j)
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time',  'Position', 'x_Li', 'x_BMP', 'Flux_Li', 'Flux_BMP', 'Phi'
    ...
    do j = 1, NJ                                    !
                                                          !
      write(56, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j), cprev(4,j), cprev(5,j)
...
module GOV_EQNS
  ...
  type(dual) :: x_Li, dcdx_Li, d2cdx2_Li
  type(dual) :: x_BMP, dcdx_BMP, d2cdx2_BMP
  ! type(dual) :: x_TFSI, dcdx_TFSI, d2cdx2_TFSI
  type(dual) :: Phi, dPhidx, d2Phidx2
  type(dual) :: N_Li, dNdx_Li     ! Fluxes
  type(dual) :: N_BMP, dNdx_BMP     ! Fluxes
  ! type(dual) :: N_TFSI, dNdx_TFSI     ! Fluxes
  real       :: position
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    d2cdx2_Li  = d2cdx2_vars_dual(1)

    x_BMP       = c_vars_dual(2)
    dcdx_BMP    = dcdx_vars_dual(2)
    d2cdx2_BMP  = d2cdx2_vars_dual(2)

    ! x_TFSI      = c_vars_dual(3)
    ! dcdx_TFSI   = dcdx_vars_dual(3)
    ! d2cdx2_TFSI = d2cdx2_vars_dual(3)

    N_Li       = c_vars_dual(3)
    dNdx_Li    = dcdx_vars_dual(3)

    N_BMP       = c_vars_dual(4)
    dNdx_BMP    = dcdx_vars_dual(4)

    ! N_TFSI      = c_vars_dual(6)
    ! dNdx_TFSI   = dcdx_vars_dual(6)
    ...
    !             dN/dη = 2η dc/dη
    GOV_(1)    = 2*position*dcdx_Li - dNdx_Li
    GOV_(2)    = 2*position*dcdx_BMP - dNdx_BMP

    !             Σ x_i = 1
    GOV_(3)    = x_Li + x_BMP + x_TFSI - 1.0

    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    !           only the diffusion coefficients with H2O are significant
    GOV_(4)    = dcdx_Li + z_Li*Fconst*x_Li/(Rigc*Temp)*dPhidx &
                & - D_LiTFSI/D_LiTFSI*(x_Li * N_TFSI - x_TFSI * N_Li) - D_LiTFSI/D_LiBMP*(x_Li * N_BMP - x_BMP * N_Li)
    GOV_(5)   = dcdx_BMP + z_BMP*Fconst*x_BMP/(Rigc*Temp)*dPhidx &
                & - D_LiTFSI/D_BMPTFSI*(x_BMP * N_TFSI - x_TFSI * N_BMP) - D_LiTFSI/D_LiBMP*(x_BMP * N_Li - x_Li * N_BMP)

    if (position == xx(2)) then
      print*, xx(2), cprev(1,2), cprev(2,2), cprev(3,2), cprev(4,2), cprev(5,2)
      print*, dcdx_Li%x, dcdx_BMP%x, dNdx_Li%x, dNdx_BMP%x, dPhidx%x
    end if
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    ! O2 + 4e- + 2*H2O --> 4 OH-
    ! x_O2 = 0.0
    BC_WEST_(1) = x_Li - xbulk_Li*0.5
    BC_WEST_(2) = N_BMP - 0.0
    ! BC_WEST_(3) = x_TFSI - 0.5

    !             Σ x_i = 1
    BC_WEST_(3)    = x_Li + x_BMP + x_TFSI - 1.0

    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    BC_WEST_(4)   = dcdx_Li + z_Li*Fconst*x_Li/(Rigc*Temp)*dPhidx &
                    & - D_LiTFSI/D_LiTFSI*(x_Li * N_TFSI - x_TFSI * N_Li) - D_LiTFSI/D_LiBMP*(x_Li * N_BMP - x_BMP * N_Li)
    BC_WEST_(5)   = dcdx_BMP + z_BMP*Fconst*x_BMP/(Rigc*Temp)*dPhidx &
                    & - D_LiTFSI/D_BMPTFSI*(x_BMP * N_TFSI - x_TFSI * N_BMP) - D_LiTFSI/D_LiBMP*(x_BMP * N_Li - x_Li * N_BMP)

    !             Σ c_i z_i = 0 = Σ x_i z_i
    ! BC_WEST_(7)   = z_Li*x_Li + z_BMP*x_BMP + z_TFSI*x_TFSI - 0.0
    ! BC_WEST_(11) = Phi - 0.0
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    type(dual), dimension(N), intent(in)   :: c_vars_dual, dcdx_vars_dual
    real, intent(in)                       :: position
    type(dual) :: c_O2_accum, c_TFSI_accum
    ...
    !             dN/dη = 2η dc/dη
    BC_EAST_(1) = 2*position*dcdx_Li - dNdx_Li
    BC_EAST_(2) = 2*position*dcdx_BMP - dNdx_BMP
    ! BC_EAST_(3) = x_TFSI - 0.5

    !             Σ x_i = 1
    BC_EAST_(3)    = x_Li + x_BMP + x_TFSI - 1.0

    ! x_i = x_i_bulk
    ! Φ = 0.0
    BC_EAST_(4)   = x_Li - xbulk_Li
    ! BC_EAST_(5)   = x_BMP - xbulk_BMP
    BC_EAST_(5)   = Phi - 0.0
...
subroutine initial_condition()
  ...
  cprev(1,:) = 0.25!xbulk_Li*0.5 + xbulk_Li*0.5 * xx/xmax
  cprev(2,:) = 0.25!xbulk_BMP
  ! cprev(3,:) = xbulk_TFSI
...
!---------------------
    ...
    elemental function acos_d(u) result(res)
        ...
        if (u%x == 1.0 .or. u%x == -1.0) then
            res%dx = set_Lin()  ! Undefined derivative
        else
  ...
  !-----------------------------------------
    ...
    elemental function set_LiN() result(res)
        real :: res

        res = sqrt(negative_one)

    end function set_LiN
