...
module user_input
  ...
  real, parameter :: xbulk_Na = 1.0e-3 / c_total
  real, parameter :: xbulk_Cl = 1.0e-3 / c_total
  real, parameter :: xbulk_H2O = 1.0 - xbulk_O2 - xbulk_Na - xbulk_Cl
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  'Position', 'x_O2', 'x_H2O', 'Flux_O2', 'Flux_H2O'
      write(*, header_fmt) 'hours', 'ξ'       , ''    , ''     , ''       , ''
    end if

    do j = 1, NJ                                    !
                                                          !
      write(*, data_fmt) t_write, xx(j), cprev(1,j)*55.5, cprev(2,j)*55.5, cprev(3,j)*55.5, cprev(4,j)*55.5, cprev(5,j)*55.5, &
                          & cprev(11,j)
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time',  'Position', 'x_O2', 'x_OH', 'x_Na', 'x_Cl', 'x_H2O', &
      & 'Flux_O2', 'Flux_OH', 'Flux_Na', 'Flux_Cl', 'Flux_H2O'
      write(56, header_fmt) 'hours', 'ξ'       , ''    , ''    , ''    , ''    , ''     , &
      & ''       , ''       , ''       , ''       , ''
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
                                                          !
      write(56, data_fmt) t_write, xx(j), cprev(1,j), cprev(2,j), cprev(3,j), cprev(4,j), cprev(5,j), cprev(6,j), cprev(7,j), &
                          & cprev(8,j), cprev(9,j), cprev(10,j)
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    x_OH       = c_vars_dual(2)
    dcdx_OH    = dcdx_vars_dual(2)
    d2cdx2_OH  = d2cdx2_vars_dual(2)
    ...
    x_Cl       = c_vars_dual(4)
    dcdx_Cl    = dcdx_vars_dual(4)
    d2cdx2_Cl  = d2cdx2_vars_dual(4)

    x_H2O      = c_vars_dual(5)
    dcdx_H2O   = dcdx_vars_dual(5)
    d2cdx2_H2O = d2cdx2_vars_dual(5)

    N_O2       = c_vars_dual(6)
    dNdx_O2    = dcdx_vars_dual(6)

    N_OH       = c_vars_dual(7)
    dNdx_OH    = dcdx_vars_dual(7)

    N_Na       = c_vars_dual(8)
    dNdx_Na    = dcdx_vars_dual(8)

    N_Cl       = c_vars_dual(9)
    dNdx_Cl    = dcdx_vars_dual(9)

    N_H2O      = c_vars_dual(10)
    dNdx_H2O   = dcdx_vars_dual(10)

    Phi        = c_vars_dual(11)
    dPhidx     = dcdx_vars_dual(11)
    d2Phidx2   = d2cdx2_vars_dual(11)

    !             dN/dη = 2η dc/dη
    GOV_(1)    = 2*position*dcdx_O2 - dNdx_O2
    GOV_(2)    = 2*position*dcdx_OH - dNdx_OH
    GOV_(3)    = 2*position*dcdx_Na - dNdx_Na
    GOV_(4)    = 2*position*dcdx_Cl - dNdx_Cl
    GOV_(5)    = 2*position*dcdx_H2O - dNdx_H2O

    !             Σ x_i = 1
    GOV_(6)    = x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0

    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    !           only the diffusion coefficients with H2O are significant
    GOV_(7)    = dcdx_O2 + z_O2*Fconst*x_O2/(Rigc*Temp)*dPhidx - D_O2/D_O2*(x_O2 * N_H2O - x_H2O * N_O2)
    GOV_(8)    = dcdx_OH + z_OH*Fconst*x_OH/(Rigc*Temp)*dPhidx - D_O2/D_OH*(x_OH * N_H2O - x_H2O * N_OH)
    GOV_(9)    = dcdx_Na + z_Na*Fconst*x_Na/(Rigc*Temp)*dPhidx - D_O2/D_Na*(x_Na * N_H2O - x_H2O * N_Na)
    GOV_(10)   = dcdx_Cl + z_Cl*Fconst*x_Cl/(Rigc*Temp)*dPhidx - D_O2/D_Cl*(x_Cl * N_H2O - x_H2O * N_Cl)

    !             Σ c_i z_i = 0 = Σ x_i z_i
    ! GOV_(11)   = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O - 0.0
    GOV_(11) = Phi - 0.0
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    ! N_i = s_i/s_O2 * N_O2
    BC_WEST_(1) = x_O2 - 0.0
    BC_WEST_(2) = N_OH - s_OH/s_O2 * N_O2
    BC_WEST_(3) = N_Na - s_Na/s_O2 * N_O2
    BC_WEST_(4) = N_Cl - s_Cl/s_O2 * N_O2
    BC_WEST_(5) = N_H2O - s_H2O/s_O2 * N_O2

    !             Σ x_i = 1
    BC_WEST_(6)    = x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0

    !           dx_i/dη + z_i F x_i / RT * dΦ/dη = Σ D_OR / D_ij * (x_i N_j - x_j N_i)
    BC_WEST_(7)    = dcdx_O2 + z_O2*Fconst*x_O2/(Rigc*Temp)*dPhidx - D_O2/D_O2*(x_O2 * N_H2O - x_H2O * N_O2)
    ! BC_WEST_(7)    = dcdx_O2 + x_H2O * N_O2
    BC_WEST_(8)    = dcdx_OH + z_OH*Fconst*x_OH/(Rigc*Temp)*dPhidx - D_O2/D_OH*(x_OH * N_H2O - x_H2O * N_OH)
    BC_WEST_(9)    = dcdx_Na + z_Na*Fconst*x_Na/(Rigc*Temp)*dPhidx - D_O2/D_Na*(x_Na * N_H2O - x_H2O * N_Na)
    BC_WEST_(10)   = dcdx_Cl + z_Cl*Fconst*x_Cl/(Rigc*Temp)*dPhidx - D_O2/D_Cl*(x_Cl * N_H2O - x_H2O * N_Cl)

    !             Σ c_i z_i = 0 = Σ x_i z_i
    ! BC_WEST_(11)   = z_O2*x_O2 + z_OH*x_OH + z_Na*x_Na + z_Cl*x_Cl + z_H2O*x_H2O - 0.0
    BC_WEST_(11) = Phi - 0.0
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    ...
    !             dN/dη = 2η dc/dη
    BC_EAST_(1) = 2*position*dcdx_O2 - dNdx_O2
    BC_EAST_(2) = 2*position*dcdx_OH - dNdx_OH
    BC_EAST_(3) = 2*position*dcdx_Na - dNdx_Na
    BC_EAST_(4) = 2*position*dcdx_Cl - dNdx_Cl
    BC_EAST_(5) = 2*position*dcdx_H2O - dNdx_H2O

    !             Σ x_i = 1
    BC_EAST_(6)    = x_O2 + x_OH + x_Na + x_Cl + x_H2O - 1.0

    ! x_i = x_i_bulk
    ! Φ = 0.0
    BC_EAST_(7)    = x_O2 - xbulk_O2
    BC_EAST_(8)    = x_OH - xbulk_OH
    BC_EAST_(9)    = x_Na - xbulk_Na
    BC_EAST_(10)   = x_Cl - xbulk_Cl
    BC_EAST_(11)   = Phi - 0.0
...
subroutine initial_condition()
  ...
  cprev(1,:) = xbulk_O2 * xx / xmax
  cprev(2,:) = xbulk_O2 * (xmax - xx)/xmax
  cprev(3,:) = xbulk_Na
  cprev(4,:) = xbulk_Cl
  cprev(5,:) = xbulk_H2O
