...
module user_input
  ...
  real, parameter :: cbulk_H2O = c_total - cbulk_O2
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  'Position', 'Conc'
      write(*, header_fmt) 'hours', 'ξ',        'mol/cm3'
    end if

    do j = 1, NJ                                    !
                                                          !
      write(*, data_fmt) t_write, xx(j), cprev(1,j)
...
!           write the headers on the first entrance into write all voltage
      ...
      write(56, header_fmt) 'hours', 'ξ',        'mol/cm3'
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
                                                          !
      write(56, data_fmt) t_write, xx(j), cprev(1,j)
...
module GOV_EQNS
  ...
  type(dual) :: c_O2, dcdx_O2, d2cdx2_O2
  type(dual) :: c_H2O, dcdx_H2O, d2cdx2_H2O
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)  result(GOV_)
    ...
    c_H2O      = c_vars_dual(2)
    dcdx_H2O   = dcdx_vars_dual(2)
    d2cdx2_H2O = d2cdx2_vars_dual(2)

    N_O2       = c_vars_dual(3)
    dNdx_O2    = dcdx_vars_dual(3)

    N_H2O      = c_vars_dual(4)
    dNdx_H2O   = dcdx_vars_dual(4)

    c_dmudx    = d_chemical_potential_dx(c_vars_dual, dcdx_vars_dual)
    c_dmudx_O2 = c_dmudx(1)

    GOV_(1)    = c_dmudx_O2 - Rigc*Temp/(c_total * D_O2) * (c_O2 * N_H2O - c_H2O * N_O2)
    GOV_(2)    = c_O2 + c_H2O - c_total
    GOV_(3)    = -dNdx_O2
    GOV_(4)    = -dNdx_H2O
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual, position) result(BC_WEST_)
    ...
    BC_WEST_(1) = c_dmudx_O2 - Rigc*Temp/(c_total * D_O2) * (c_O2 * N_H2O - c_H2O * N_O2)
    BC_WEST_(2) = c_O2 + c_H2O - c_total
    BC_WEST_(3) = c_O2 - 0.0
    BC_WEST_(4) = N_H2O - 0.0

  end function Boundary_WEST

  function Boundary_EAST (c_vars_dual, dcdx_vars_dual, position) result(BC_EAST_)
    type(dual), dimension(N)               :: BC_EAST_, accum_temp, gov_temp, d2cdx2_vars_dual = 0.0
    type(dual), dimension(N), intent(in)   :: c_vars_dual, dcdx_vars_dual
    real, intent(in)                       :: position
    ...
    ! accum_temp = ACCUM(c_vars_dual, position)
    ! gov_temp   = GOV(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual, position)

    BC_EAST_(1) = c_O2 - cbulk_O2
    BC_EAST_(2) = c_O2 + c_H2O - c_total
    BC_EAST_(3) = c_O2_accum/delT + dNdx_O2
    BC_EAST_(4) = c_H2O_accum/delT + dNdx_H2O
