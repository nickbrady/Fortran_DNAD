...
module user_input
  ...
  real               :: time                             ! [s]
  real, parameter    :: xmax = 23.2e-4 *5.5                      ! 100 um
...
!           write the headers on the first entrance into write all voltage
    ...
    do j = 1, NJ                                    !
      ...
      xi       = zeta * (aa * viscosity / 3.0 / D_O2)**(1./3.)
                                                    !
      write(56, data_fmt) t_write, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, vel, xi, zeta
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual, position)  result(Flux_)
    ...
    Flux_(1) = -D_O2 * dcdx_O2 + vel * c_O2 - z_O2 * u_O2 * Fconst * c_O2 * dPhidx
    Flux_(2) = -D_OH * dcdx_OH + vel * c_OH - z_OH * u_OH * Fconst * c_OH * dPhidx
    Flux_(3) = -D_Na * dcdx_Na + vel * c_Na - z_Na * u_Na * Fconst * c_Na * dPhidx
    Flux_(4) = -D_Cl * dcdx_Cl + vel * c_Cl - z_Cl * u_Cl * Fconst * c_Cl * dPhidx
    Flux_(5) = 0.0
