...
module user_input
  ...
  real               :: delT = 1.0e3                       ! size of timestep [s]
  real               :: time                             ! [s]
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      ...
      ! print*,
      write(*, data_fmt) t_write, conc_1, conc_NJ, vel_1, vel_NJ, xi, zeta
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'C_O2', 'C_OH',    'C_Na',    'C_Cl',    'Phi',   'Velocity'
    ...
    do j = 1, NJ                                    !
      ...
      ! xi       = zeta * (aa * viscosity / 3.0 / D_O2)**(1./3.)
                                                    !
      write(56, data_fmt) t_write, xx(j), c_O2, c_OH, c_Na, c_Cl, Phi, vel
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual, position)  result(Flux_)
    ...
    vel      = velocity(position)

    Flux_(1) = -D_O2/D_O2 * dcdx_O2 + vel * c_O2 - z_O2 * u_O2/D_O2 * Fconst * c_O2 * dPhidx
    Flux_(2) = -D_OH/D_O2 * dcdx_OH + vel * c_OH - z_OH * u_OH/D_O2 * Fconst * c_OH * dPhidx
    Flux_(3) = -D_Na/D_O2 * dcdx_Na + vel * c_Na - z_Na * u_Na/D_O2 * Fconst * c_Na * dPhidx
    Flux_(4) = -D_Cl/D_O2 * dcdx_Cl + vel * c_Cl - z_Cl * u_Cl/D_O2 * Fconst * c_Cl * dPhidx
    Flux_(5) = 0.0


  end function FLUX

  function RXN(c_vars_dual, position)                   result(Rxn_)
    ...
    Rxn_(1) = +c_vars_dual(1) * vel * 2 / position
    Rxn_(2) = +c_vars_dual(2) * vel * 2 / position
    Rxn_(3) = +c_vars_dual(3) * vel * 2 / position
    Rxn_(4) = +c_vars_dual(4) * vel * 2 / position
    Rxn_(5) = z_O2 * c_O2 + z_OH * c_OH + z_Na * c_Na + z_Cl * c_Cl
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    flux_temp = FLUX(c_vars_dual, dcdx_vars_dual, position)
    rxn_temp  = RXN(c_vars_dual, position)

    BC_WEST_(1) = c_O2 - 0.0
    BC_WEST_(2) = flux_temp(2) - s_OH / s_O2 * flux_temp(1)
    BC_WEST_(3) = flux_temp(3) - s_Na / s_O2 * flux_temp(1)
    BC_WEST_(4) = flux_temp(4) - s_Cl / s_O2 * flux_temp(1)
...
subroutine auto_fill(j)
  ...
  if (j /= 1) then                          !-----------------------------------
    ...
    alphaW = 0.0                            ! from from west to east
    do ic=1,N                               !-----------------------------------
  ...
  !-----------------------------------------------------------------------------
    ...
    flux_dualW = FLUX(cW_dual, dcdxW_dual, xx(j) - delx(j)/2)    ! West Side Flux
    flux_dualE = FLUX(cE_dual, dcdxE_dual, xx(j) + delx(j)/2)    ! East Side Flux

    cj_dual = c_to_dual(cprev(:,j))           !---------------------------------
    reaction_dual = RXN(cj_dual, xx(j))              ! Control Volume - Rxn, Accum
