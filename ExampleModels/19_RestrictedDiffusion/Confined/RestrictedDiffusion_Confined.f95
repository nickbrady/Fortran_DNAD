...
module user_input
  ...
  real :: i_applied_cm2 = 2*Fconst*D_bulk*cbulk_Li / xmax /1.5          ! 1 μA/cm2
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      ...
      write(*, data_fmt) t_write, conc_Li, cprev(1,NJ)
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'Delta_Conc'
      write(56, header_fmt) 'hours', 'cm',      'mol/cm3'
                                                      !
    end if                                              !
...
! ******************************************************************************
  ...
  function Cross_Sectional_Area(Geometry) result(Cross_Area)
    ...
    if (Geometry == 'Rectangular') then
      ...
      do j = 40-1, 60
        Cross_Area(2,j) = 1.0 * 0.1
      end do

      do j = 40, 60+1
        Cross_Area(1,j) = 1.0 * 0.1
      end do
...
subroutine initial_condition()
  ...
  ! do j = 1, NJ
  cprev(1, :) = cbulk_Li
  ! cprev(1,:40) = xx(:40)/xx(40) * cbulk_Li
  ! cprev(1,41:60) = cbulk_Li
  ! cprev(1,61:) = cbulk_Li + (xx(61:) - xx(60))/(xx(NJ) - xx(60)) * cbulk_Li
  ! end do
