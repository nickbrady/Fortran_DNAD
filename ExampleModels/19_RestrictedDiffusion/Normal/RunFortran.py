...
fortran_template = 'RestrictedDiffusion.f95'
fortran_vals = 'RestrictedDiffusion_vals.f95'
...
for percent in [1, 0.5, 0.1, 0.01]:
    ...
    sed('_percent_max_', str(percent), fortran_template, dir__+'/'+fortran_vals)

    os.chdir(dir__)

    # sort the modules in the fortran file
    p = call(['SortFortranModules.py', fortran_vals])
    _sorted_fortran_template = 'ReSort' + fortran_vals
