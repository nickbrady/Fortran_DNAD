...
module user_input
  ...
  integer, parameter :: Numbertimesteps = 100 * 3.6e3       ! Number of time steps
...
contains
  ...
  subroutine write_condition(it)
    ...
    real :: write_every_x_sec = 60           ! 3600 s = 1 hour
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      ...
      write(*, data_fmt) t_write, conc_Li_1, conc_Li_NJ, Pot_1, Pot_NJ, Delta_Volt, diff_conc(cbulk_Li)
...
subroutine initial_condition()
  ...
  ! do j = 1, NJ
    cprev(1,:) = xx/xmax * cbulk_Li * 2 * 0.1 + (cbulk_Li * (1 - 0.1))
  ! end do
