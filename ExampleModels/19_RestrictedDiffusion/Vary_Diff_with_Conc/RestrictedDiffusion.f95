...
module user_input
  ...
  integer, parameter :: Numbertimesteps = 200 * 3.6e3       ! Number of time steps
  ...
  real, parameter :: D_bulk   = 2e-6
  real, parameter :: D_slow   = 1e-6/100
  real, parameter :: z_Li     = +1.0
  real, parameter :: nu_Li    = 1.0
  real, parameter :: s_Li     = +1.0

  real :: i_applied_cm2 = 0           ! 1 μA/cm2
...
module write_data_mod
  ...
  real :: conc_Li, conc_Li_1, conc_Li_NJ
  real :: Pot_1, Pot_NJ, Delta_Volt

contains

  function Nernst_Potential(conc)     Result(U_Nernst)
    real :: U_Nernst
    real, intent(in) :: conc
    real :: U_ref, conc_ref
    ...
    U_Nernst = U_ref - Rigc*Temp/Fconst*LOG(conc/conc_ref)

  end function Nernst_Potential
  ...
  subroutine write_condition(it)
    ...
    real :: write_every_x_sec = 3600*0.1           ! 3600 s = 1 hour
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  'conc_1'
      write(*, header_fmt) 'hours', 'mol/cm3'
    end if

      conc_Li_1     = cprev(1,1)
      conc_Li_NJ    = cprev(1,NJ)
      Pot_1         = Nernst_Potential(conc_Li_1)
      Pot_NJ        = Nernst_Potential(conc_Li_NJ)
      Delta_Volt    = ABS(Pot_NJ - Pot_1)

      write(*, data_fmt) t_write, conc_Li_1, conc_Li_NJ, Pot_1, Pot_NJ, Delta_Volt, diff_conc(cbulk_Li), cbulk_Li
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'Conc',     'Potential'
      write(56, header_fmt) 'hours', 'cm',      'mol/cm3',  'Volts'
                                                      !
    end if                                              !
    ...
    do j = 1, NJ                                    !
      conc_Li    = cprev(1,j)
      Pot_1      = Nernst_Potential(conc_Li)
                                                    !
      write(56, data_fmt) t_write, xx(j), conc_Li, Pot_1
...
module GOV_EQNS
  ...
  public diff_conc
  interface diff_conc
      module procedure diff_conc_real
      module procedure diff_conc_dual ! obtain sine of a dual number, elemental
  end interface

contains

  function diff_conc_dual(c_vars_dual)   Result(Diff_)
    type(dual)                            :: Diff_
    type(dual), dimension(N), intent(in)  :: c_vars_dual
    type(dual)                            :: conc

    conc = c_vars_dual(1)
    ! Diff_ = D_bulk * (1 - TANH(1*conc/cbulk_Li - 1)) + D_slow
    ! Diff_ = D_bulk * (1 - ERF(conc/cbulk_Li - 2))/2 + D_slow
    Diff_ = D_bulk * (1 - ERF(2*(conc/cbulk_Li - 2) + 2))/2 + D_slow
    ! print*, time/3600., conc%x/cbulk_Li, Diff_%x

    ! print*, D_bulk * (1 - ERF(2*(cbulk_Li/cbulk_Li - 2) + 2))/2 + D_slow, cbulk_Li

  end function diff_conc_dual

  function diff_conc_real(c_Li)   Result(Diff_)
    real                            :: Diff_
    real, intent(in)  :: c_Li
    real                            :: conc
    ...
    conc = c_Li
    ! Diff_ = D_bulk * (1 - TANH(1*conc/cbulk_Li - 1)) + D_slow
    ! Diff_ = D_bulk * (1 - ERF(c_Li/cbulk_Li - 2))/2 + D_slow
    Diff_ = D_bulk * (1 - ERF(2*(conc/cbulk_Li - 2) + 2))/2 + D_slow

  end function diff_conc_real
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual, position)  result(Flux_)
    ...
    diff_Li = diff_conc(c_vars_dual)

    Flux_(1) = -diff_Li * dcdx_Li
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = z_Li*nu_Li*flux_temp(1) - i_applied_cm2 * (1 - trans_Li)/Fconst
  ...
  do it = 1, Numbertimesteps
      ...
      ! **********************************************************************
        ...
        if (time >= 100*3600) then
          ...
          i_applied_cm2 = cbulk_Li*D_bulk * Fconst * 1.0
        end if
...
subroutine initial_condition()
  ...
  ! do j = 1, NJ
    cprev(1,:) = cbulk_Li !xx/xmax * cbulk_Li * 2 * _percent_max_ + (cbulk_Li * (1 - _percent_max_))
  ! end do
