...
module user_input
  ...
  integer, parameter :: maxIterations = 1e3
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time', 'Position', 'ω_e',  'vel'
    ...
    do j = 1, NJ                                    !
      ...
      vel    = cprev(2,j)
                                                    !
      write(56, data_fmt) t_write, xx(j), w_e, vel
...
module GOV_EQNS
  ...
  type(dual) :: w_e, w_0
  type(dual) :: dwdx_e, dwdx_0
  type(dual) :: d2wdx2_e, d2wdx2_0
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function GOV_RHS(c_vars_dual, dcdx_vars_dual, d2cdx2_vars_dual)  result(GOV_)
    ...
    GOV_(1)     = density * eps * Diff * d2wdx2_e - density * vel * dwdx_e
    GOV_(2)     = dvel_dx
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    BC_WEST_(1) = flux_e - 1e-8
    BC_WEST_(2) = flux_0 - 0.0
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = flux_e - 1e-8
    BC_EAST_(2) = flux_0 - 0.0
