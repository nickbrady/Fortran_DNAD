...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it == 0) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'State', 'Time', 'ω_e_1', 'ω_e_NJ', 'vel_1', 'vel_NJ', 'MassBal_0', 'MassBal_e'
...
!           write the headers on the first entrance into write all voltage
    ...
    do j = 1, NJ                                    !           !
      ...
      flux_e = -density/MW_e * Diff * dwdx_e + density*w_e/MW_e * vel
...
! (1) ∂(ρ/MW₀ ω₀)/∂t = -∇⋅(-ρ/MWₑ D ∇ωₑ + ρ ωₑ/MWₑ⋅𝐯)
! 𝐍 = -ρ/MWₑ D ∇ωₑ + ρ ωₑ/MWₑ⋅𝐯
! R = 0
! Accum = ρ/MWₑ ωₑ/Δt

! (2) ∂(ρ/MW₀ ω₀)/∂t = -∇⋅(-ρ/MW₀ D ∇ω₀ + ρ ω₀/MW₀⋅𝐯)
! 𝐍 = -ρ/MW₀ D ∇ω₀ + ρ ω₀/MW₀⋅𝐯
! R = 0
! Accum = ρ/MW₀ ω₀/Δt
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ! (1)   N_1 = -ρ/MWₑ D ∇ωₑ + ρ ωₑ/MWₑ⋅𝐯
    ! (2)   N_mass = -ρ/MW₀ D ∇ω₀ + ρ ω₀/MW₀⋅𝐯
    ...
    FLUX_(1) = -density/MW_e * Diff * dwdx_e + density*w_e/MW_e * vel
    FLUX_(2) = -density/MW_0 * Diff * dwdx_0 + density*w_0/MW_0 * vel
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    BC_WEST_(1) = flux_temp(1) - 1.0e-7*10
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = flux_temp(1) - 1.0e-7*10
