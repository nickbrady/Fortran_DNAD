...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it.EQ.1) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'State', 'Time', 'c_A_1', 'c_A_NJ', 'vel_1', 'vel_NJ', 'MB_0', 'MB_e', 'Φ_2(NJ)'
      write(*, header_fmt) 'CDR',  'hours', 'mol/L',  'cm/s' , 'cm/s' , 'cm/s', 'MB_0', 'MB_e', 'Φ_2(NJ)'
    end if
    ...
    write(*, data_fmt) state, t_write, cprev(1,1), cprev(1,NJ), cprev(2,1), cprev(2,NJ), &
                      & mass_balance_0, mass_balance_e, cprev(3,NJ)
