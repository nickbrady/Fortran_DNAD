...
contains
  ...
  subroutine write_positional_information_to_file(it)
    ...
    integer :: it, j
    real :: dwdx_e, dwdx_0
    real :: flux_e, flux_0, Phi_2
    real :: flux_cat, flux_an, i2
...
!           write the headers on the first entrance into write all voltage
    ...
    do j = 1, NJ
      ...
      ! c_vars_dual = c_to_dual(cprev(:,j))
      ! dcdx_vars_dual = dcdx_to_dual(dcdx)
      ! if (j==1) then
      !   Phi_2_dual = 0.0
      ! else
      !   Phi_2_dual = Phi_2_dual + dPhi2_dx_function(c_vars_dual)
      ! end if
      Phi_2 = 0.0
      ...
      write(56, data_fmt) state,    t_write,   xx(j),  w_e, vel, Phi_2, Peclet_Number, &
                        & flux_e, flux_0, flux_cat, flux_an, i2
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    FLUX_(1) = -nu_cat*density/MW_e * Diff * dwdx_e + t_cat/(z_cat*Fconst)*i_app_cm2 + nu_cat*density*w_e/MW_e * vel
  ...
  function ACCUM(c_vars_dual) result(Accum_)
    ...
    Accum_(1) = density*w_e/delT
    Accum_(2) = density*w_0/delT
