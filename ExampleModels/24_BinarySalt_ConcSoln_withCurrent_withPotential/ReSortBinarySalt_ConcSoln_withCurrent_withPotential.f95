...
module user_input
  ...
  real :: Diff = 1e-6
  real :: kappa = 10e-3     ! mS/cm

  real :: i_app_cm2 = 1.1e-6 !* Fconst      ! applied current

  !electrochemical reaction
  real :: s_0   = 0       ! no side reaction
  real :: s_cat = 0     ! no reactions except at boiundaries
  real :: s_an  = 0
...
!           write the headers on the first entrance into write all voltage
    write(56, header_fmt) 'State', 'Time', 'Position', 'ω_e', 'velocity', 'Peclet', 'flux_e', 'flux_0'
    write(56, header_fmt) 'CDR',  'hours',  'um'     , '-',   'cm/s'    , '-', 'mol/cm2/s', 'mol/cm2/s'
                                                    !           !
    end if                                          !           !
    ...
    do j = 1, NJ
      ...
      write(56, data_fmt) state,    t_write,   xx(j),  w_e, vel, Peclet_Number, flux_e, flux_0
...
! μ_i = RT ln λ_i
! λ_i = m γ_i λ_i^θ
! μ_e = μ_e^θ + νRT ln(m_e γ+-) + RT ln(ν_+^ν_+ ν_-^ν_-)
  function chemical_potential_c_e(c_e)        result(chem_pot_e)
    type(dual)                :: chem_pot_e
    type(dual), intent(in)    :: c_e          ! concentration fraction of salt A
    type(dual)                :: m_e          ! molality : mol_e / kg_0

    chem_pot_e = Rigc * Temp * log(c_e ** nu_e)

  end function chemical_potential_c_e


  function chemical_potential_x_e(x_e)        result(chem_pot_e)
    type(dual)                :: chem_pot_e
    type(dual), intent(in)    :: x_e          ! mol fraction of salt A

    ! μ_i = RT ln λ_i
    ! λ_i = m γ_i λ_i^θ
    ! μ_e = RT ln (x_e^ν_e)  =  ν_e RT ln (x_e)
    chem_pot_e = Rigc * Temp * log(x_e ** nu_e)

  end function chemical_potential_x_e

  ! μ_e = RT ln λ_e
  ! μ_e = μ_e^θ + νRT ln(m_e γ+-) + RT ln(ν_+^ν_+ ν_-^ν_-)
  function chemical_potential_w_e(w_e)        result(chem_pot_e)
    type(dual)                :: chem_pot_e
    type(dual), intent(in)    :: w_e          ! mass fraction of salt A
    type(dual)                :: c_e, m_e          ! molality : mol_e / kg_0
    type(dual)                :: w_0
    real                      :: gamma_e = 1

    w_0 = 1 - w_e

    c_e = density * w_e / MW_e
    m_e = c_e / (density * w_0)

    chem_pot_e = nu_e * Rigc * Temp * log(m_e * gamma_e)

  end function chemical_potential_w_e



  function dmudx_e(c_vars_dual, dcdx_vars_dual)        result(d_dx_chem_pot_e)
    ...
    ! ρ = ρ_e + ρ_0
    ! ρ_i = c_i * MW_i = ρ(1 - ω)
    type(dual)                :: d_dx_chem_pot_e
    type(dual), dimension(N), intent(in)    :: c_vars_dual, dcdx_vars_dual          ! mass fraction of salt A
    type(dual)                :: w_e, dwdx_e
    type(dual)                :: w_0
    type(dual)                :: c_e, m_e          ! concentration, molality : mol_e / kg_0
    real                      :: gamma_e = 1
    type(dual)                :: density_0
    ...
    d_dx_chem_pot_e = nu_e * Rigc * Temp /density_0 * dwdx_e/w_e

  end function dmudx_e
!   ! du_A = u_A%dx * dxA_dx
!
...
!
  function solution_current_i2(c_vars_dual, dcdx_vars_dual)  Result(i_2)
    type(dual)                :: i_2
    type(dual), dimension(N), intent(in)    :: c_vars_dual, dcdx_vars_dual
    type(dual)  :: w_e, w_0, Phi_1, Phi_2
    type(dual)  :: dwdx_e, dwdx_0, dPhi_1dx, dPhi2_dx
    type(dual)  :: dmu_dx_e
    type(dual)  :: c_e, c_0
    ...
    c_e = density * w_e / MW_e
    c_0 = density * w_0 / MW_0

    dmu_dx_e = dmudx_e(c_vars_dual, dcdx_vars_dual)
    ! i = -κ∇Φ_2 - κ/F*(s_+/(n ν_+) - s_0 c_0 / (n c_e) + t_+/(ν_+ z_+)) ∇μ_e
    i_2 = -kappa * dPhi2_dx - kappa/Fconst*(s_cat/(n*nu_cat) - s_0*c_0/(n*c_e) + t_cat/(nu_cat*z_cat))*dmu_dx_e

  end function solution_current_i2
!
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    type(dual), dimension(N), intent(in)  :: c_vars_dual, dcdx_vars_dual
    type(dual)                            :: i_2_sol_curr
    type(dual)                            :: Flux_an, Flux_cat
    ...
    i_2_sol_curr = solution_current_i2(c_vars_dual, dcdx_vars_dual)

    FLUX_(1) = -density/MW_e * Diff * dwdx_e + t_cat/(nu_cat*z_cat)*i_2_sol_curr/Fconst + w_e/MW_e * vel
    FLUX_(2) = -density/MW_0 * Diff * dwdx_0 &
              & - 1/MW_0*(MW_cat/z_cat * t_cat + MW_an/z_an * t_an)*i_2_sol_curr/Fconst  &
              & + w_0/MW_0 * vel

    Flux_cat = nu_cat * FLUX_(1)
    Flux_an  = -nu_an * density/MW_e * Diff * dwdx_e + t_an/(z_an*Fconst)*i_2_sol_curr + nu_an*w_e/MW_e * vel
  ...
  function ACCUM(c_vars_dual) result(Accum_)
    ...
    Accum_(1) = 1/MW_e * density*w_e/delT
    Accum_(2) = 1/MW_0 * density*w_0/delT
    Accum_(3) = 0.0
