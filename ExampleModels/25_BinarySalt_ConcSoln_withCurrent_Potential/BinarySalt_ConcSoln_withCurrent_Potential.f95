...
!           write the headers on the first entrance into write all voltage
    write(56, header_fmt) 'State', 'Time', 'Position', 'ω_e', 'velocity', 'Φ_2', 'Peclet', 'flux_e', 'flux_0'
    write(56, header_fmt) 'CDR',  'hours',  'um'     , '-',   'cm/s'    , 'Φ_2', '-', 'mol/cm2/s', 'mol/cm2/s'
                                                    !           !
    end if                                          !           !
    ...
    do j = 1, NJ
      ...
      write(56, data_fmt) state,    t_write,   xx(j),  w_e, vel, Phi_2, Peclet_Number, flux_e, flux_0
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ! (1)   N_e = N_+/ν_+ = -ρ/MW_e D dω_e/dx + (t_+ i)/(ν_+ z_+ F) + ρ ω_e/MW_e * v
    ! (2)   N_0 = -ρ/MW_0 D dω_0/dx + 1/MW_0 [MW_+/z_+ t_+ + MW_-/z_- t_-] i/F + ρ ω_0/MW_0 * v
    ! (3)   electroneutrality
    ...
    sol_curr = solution_current_i2_function(c_vars_dual, dcdx_vars_dual)

    FLUX_(1) = -density/MW_e * Diff * dwdx_e + t_cat/(nu_cat*z_cat*Fconst)*sol_curr + w_e*density/MW_e * vel
    FLUX_(2) = -density/MW_0 * Diff * dwdx_0 &
              & - 1/MW_0*(MW_cat/z_cat * t_cat + MW_an/z_an * t_an)*sol_curr/Fconst  &
              & + w_0*density/MW_0 * vel
    FLUX_(3) = 0.0
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    BC_WEST_(1) = z_cat*nu_cat*Fconst*flux_temp(1) - i_app_cm2
    BC_WEST_(2) = flux_temp(2) - 0.0
    BC_WEST_(3) = rxn_temp(3)

    ! print*, BC_WEST_(1)%x, i_app_cm2
    ! print*, flux_temp(1)%x*Fconst*z_cat*nu_cat
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = z_cat*nu_cat*Fconst*flux_temp(1) - i_app_cm2
    BC_EAST_(2) = flux_temp(2) - 0.0
    BC_EAST_(3) = Phi_2 - 0.0

    ! print*, BC_EAST_(1)%x, i_app_cm2
    ! print*, flux_temp(1)%x*Fconst*z_cat*nu_cat
