...
module user_input
  ...
  real :: i_app_cm2 = 1e-4*8 !* Fconst / 10.
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it.EQ.1) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt_screen) 'State', 'Time', 'c_A_1', 'c_A_NJ', 'Φ_1', 'Φ_NJ'
      write(*, header_fmt_screen) 'CDR',  'hours', 'mol/L',  'cm/s' ,   'V',    'V'
    end if
    ...
    write(*, data_fmt_screen) state, t_write, c_LiPF6_1*1e3, c_LiPF6_NJ*1e3, &
...
!           write the headers on the first entrance into write all voltage
    write(56, header_fmt_file) 'State', 'Time', 'Position', 'c_LiPF6', 'Φ_2'
    write(56, header_fmt_file) 'CDR',  'hours',  'um'     ,  'mol/mL', 'V'
                                                    !           !
    end if                                          !           !
    ...
    do j = 1, NJ
      ...
      c_LiPF6   = cprev(1,j)
      Phi_2     = cprev(2,j) - Phi_2_NJ

      write(56, data_fmt_file) state,    t_write,   xx(j),  c_LiPF6, Phi_2
...
contains
  ...
  function diffusion_Li(c_LiPF6)    result(D_Li)
    type(dual), intent(in)    :: c_LiPF6
    type(dual)                :: D_Li
    type(dual)                :: D_salt, t_Li

    D_salt = diffusion_coef_LiPF6_in_EC_EMC(c_LiPF6)
    t_Li   = transference_number(c_LiPF6)! + 0.51

    D_Li = D_salt / ((1 - z_cat/z_an)*(1 - t_Li))

  end function diffusion_Li

  function diffusion_PF6(c_LiPF6)    result(D_PF6)
    type(dual), intent(in)    :: c_LiPF6
    type(dual)                :: D_PF6
    type(dual)                :: D_salt, t_Li
    ...
    D_PF6 = D_salt / ((1 - z_cat/z_an)*(t_Li))

  end function diffusion_PF6
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual)  result(Flux_)
    ...
    type(dual), dimension(N), intent(in)  :: c_vars_dual, dcdx_vars_dual
    type(dual) :: D_Li, D_PF6, u_Li, u_PF6

    c_LiPF6     = c_vars_dual(1)
    dcdx_LiPF6  = dcdx_vars_dual(1)
    dPhi2_dx    = dcdx_vars_dual(2)

    D_Li = diffusion_Li(c_LiPF6)
    D_PF6 = diffusion_PF6(c_LiPF6)

    u_Li  = D_Li  / (Rigc*Temp)
    u_PF6 = D_PF6 / (Rigc*Temp)

    FLUX_(1) = -D_Li  * dcdx_LiPF6 - z_cat * u_Li  * c_LiPF6 * Fconst * dPhi2_dx
    FLUX_(2) = -D_PF6 * dcdx_LiPF6 - z_an  * u_PF6 * c_LiPF6 * Fconst * dPhi2_dx
  ...
  function ACCUM(c_vars_dual) result(Accum_)
    ...
    Accum_(1) = c_LiPF6/delT
    Accum_(2) = c_LiPF6/delT
