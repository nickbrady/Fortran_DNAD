...
contains
  ...
  function diffusion_coef_LiPF6_in_EC_EMC_dual(c_LiPF6)    result(D_salt)
    ...
    if (c <= 1.5) then
	 D_salt =  -1.7619047619e-07*(c-0.0)**3 + 1.74642857143e-06*(c-0.0)**2  -4.51488095238e-06*(c-0.0) + 8.28125e-06
else if (c <= 2.5) then
	 D_salt =  -1.7619047619e-07*(c-1.5)**3 + 9.53571428571e-07*(c-1.5)**2  -4.64880952381e-07*(c-1.5) + 4.84375e-06
else 
	 D_salt =  -1.7619047619e-07*(c-2.5)**3 + 4.25e-07*(c-2.5)**2 + 9.1369047619e-07*(c-2.5) + 5.15625e-06
end if
...
! ******************************************************************************
  ...
  !   stop
  else if (any(cprev(1,1:NJ) < 0.0)) then
    call t_write__Equiv__Li_Bal_Assignment
    ! call write_all_data_to_terminal
    print*, 'c0 < 0.0'
    stop
