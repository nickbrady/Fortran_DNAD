...
module user_input
  ...
  real               :: delT = 1e0/10                       ! size of timestep [s]
  real               :: time                             ! [s]
...
module write_data_mod
  ...
  character(len=65) :: data_fmt   = '( 1(A5,1X), 3(F12.5,1X), 40(ES15.5,1X) )'
  character(len=65) :: data_fmt_2 = '( 1(A5,1X), 2(F12.5,1X), 40(ES15.5,1X) )'
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it.EQ.1) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'State', 'Time', 'c_A_1', 'c_A_NJ', 'vel_1', 'vel_NJ', 'MB_0', 'MB_e', 'Φ_2(1)'
      write(*, header_fmt) 'CDR',  'hours', 'mol/L',  'cm/s' , 'cm/s' , 'cm/s', 'MB_0', 'MB_e', 'Φ_2(1)'
    end if

    write(*, data_fmt) state, t_write, cprev(1,1), cprev(1,2), cprev(1,NJ/2), cprev(1,NJ-1), cprev(1,NJ)
  ...
  subroutine write_all_data_to_terminal
    ...
    do j = 1, NJ
      ...
      write(*, data_fmt_2) state, xx(j), Phi_2
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'State', 'Time', 'Position', 'u'
      write(56, header_fmt) 'CDR',  'hours',  'um'     , '-'
                                                    !           !
    end if                                          !           !
    ...
    do j = 1, NJ
      ...
      write(56, data_fmt) state,    t_write,   xx(j), Phi_2
...
contains
  ...
  !   ! print*, time/3600.0, i_app_cm2, i2%x, kappa*dPhi2_dx%x, kappa/Fconst*(t_cat)/(nu_cat*z_cat) * dmudx_e%x
  !
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    !           & - 1/MW_0*(MW_cat/z_cat * t_cat + MW_an/z_an * t_an)*i2/Fconst  &
    !           & + density*w_0/MW_0 * vel
    ! FLUX_(3) = i2
    ! dmudx_e = dmudx_e_function(c_vars_dual, dcdx_vars_dual)
    ! print*, time/3600.0, i2%x, dcdx_vars_dual(3)%x, c_vars_dual(3)%x, cprev(3,NJ)
    ! FLUX_(3) = -nu_an *density/MW_e * Diff * dwdx_e + t_an/(z_an*Fconst)*i_app_cm2 + nu_an *density*w_e/MW_e * vel
    ! FLUX_(3) = -density/MW_e * Diff * dwdx_e + t_cat/(nu_cat*z_cat*Fconst)*i2 + density*w_e/MW_e * vel
    ! print*, time/3600.0, i_app_cm2, i2%x!, dmudx_e%x, FLUX_(3)%x, nu_an *density/MW_e * Diff * dwdx_e%x, &
    ! & nu_an *density*w_e%x/MW_e * vel%x
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    u = c_vars_dual(1)
    dudx = dcdx_vars_dual(1)

    ! BC_WEST_(1) = nu_cat*z_cat*Fconst*flux_temp(1) - i_app_cm2
    ! BC_WEST_(2) = flux_temp(2) - 0.0
    BC_WEST_(1) = dudx - 0.0
    ! BC_WEST_(1) = flux_temp(1) - i_app_cm2
    ! BC_WEST_(3) = flux_temp(3) - 0.0!Phi_2 - 0.0
    ! BC_WEST_(3) = nu_cat*z_cat*Fconst*flux_temp(3) - i_app_cm2
    ! print*, time/3600.0, i_app_cm2, flux_temp(3)%x
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    type(dual), dimension(N), intent(in)   :: c_vars_dual, dcdx_vars_dual
    type(dual), dimension(N)               :: accum_temp
    type(dual)                             :: u, dudx
    ...
    BC_EAST_(1) = accum_temp(1) - c*dudx
    ! BC_EAST_(1) = Phi_2 - 0.0
...
! ******************************************************************************
  ...
  !   stop
  ! else if (any(cprev(1,:NJ-1) < 0.0)) then
  !   call t_write__Equiv__Li_Bal_Assignment
  !   call write_all_data_to_terminal
  !   print*, 'c0 < 0.0'
    ! stop
...
subroutine auto_fill(j)
  ...
  if (j == 1) then                              ! West Side Boundary Conditions
    ...
    ! flux_dualE = FLUX(cE_dual, dcdxE_dual)
    ! print*, time/3600, smG(3), flux_dualE(3)%x, boundary_conditionW(3)%x, fE(3,:)
                                                !-------------------------------
    ...
    ! flux_dualW = FLUX(cW_dual, dcdxW_dual)
    ! print*, time/3600, smG(3), flux_dualW(3)%x, boundary_conditionW(3)%x, fW(3,:), cprev(3,j)
