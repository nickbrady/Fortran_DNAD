...
module user_input
  ...
  real :: xmax          = 500e-4              ! 500 um is 500e-4 cm
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    Flux_(4) = -porosity *( Fconst*(z_1 * diff_Li + z_2 * diff_PF6) * dc0dx &
...
! *****************************************************************************
  if (experimental_program == 'C_200_Discharge_Rest_Charge') then
    ...
    rest_time      = 5 * 3600                  ! 200 hours of recovery time
...
! ******************************************************************************
  ...
  if (trim(geometry) == 'Rectangular') then
    applied_current_A = applied_specific_current/1000 * (1 - porosity) * density_Fe3O4 * xmax

  else if (trim(geometry) == 'Cylindrical') then
    applied_current_A = applied_specific_current/1000 * (1 - porosity) * density_Fe3O4 * xmax/2.0

  else if (trim(geometry) == 'Spherical') then
    applied_current_A = applied_specific_current/1000 * (1 - porosity) * density_Fe3O4 * xmax/3.0
