...
module user_input
  ...
  real :: z_Li = 1.0                             ! cation charge z_Li
  real :: z_PF6 = -1.0                            ! anion charge, z_PF6
...
! i.e c0 = c_vars_dual(1), dPhi2_dx = dcdx_vars_dual(2)
  ...
  function FLUX(c_vars_dual, dcdx_vars_dual) result(Flux_)
    ...
    u_Li = diff_Li / (Rigc * Temp)
    u_PF6 = diff_PF6 / (Rigc * Temp)

    Flux_(1) = -porosity**1.5 * (diff_Li * dc0dx + z_Li * u_Li * c0 * Fconst * dPhi_2dx)
    Flux_(2) = 0.0
    Flux_(3) = -(1 - porosity) * sigma * dPhi_1dx
    Flux_(4) = -porosity**1.5 * Fconst * ( (z_Li * diff_Li + z_PF6 * diff_PF6) * dc0dx &
        + (z_Li**2 * u_Li + z_PF6**2 * u_PF6) * Fconst * c0 * dPhi_2dx )
...
subroutine auto_fill(j)
  ...
  real :: theta
  real :: U_ref
  real :: dOCP_dc0, dOCP_dcx
  ! Exchange Current Density
  real :: i_ex_curr
  real :: dex_curr_dc0, dex_curr_dcx
  ! real :: alpha_a = 0.5
  ! real :: alpha_c = 0.5
  ! real :: k_rxn   = 1.0e-7
  ...
  real :: i_BV
  real :: eta!, U_ocp
  real :: deta_dPhi1, deta_dPhi2, deta_dc0, deta_dcx
  real :: di_BV_deta
  real :: di_BV_dPhi1, di_BV_dPhi2, di_BV_dc0, di_BV_dcx
  real :: spec_a
  real :: u_Li, u_PF6
  ...
  !-----------------------------------------------------------------------------
  ! Electrochemical Reactions and Derivatives
  !-----------------------------------------------------------------------------
  c0_r = cprev(1,j)
  c_x_r = cprev(2,j)
  Phi_1_r = cprev(3,j)
  ...
  ! Open Circuit Equation
  theta = c_x_r / max_Li_conc
  U_ref = 3.3
  U_ocp = U_ref + Rigc*Temp/Fconst * LOG( c0_r/cbulk * (1.0 - theta)/theta )

  dOCP_dc0 = Rigc*Temp/Fconst / c0_r
  dOCP_dcx = Rigc*Temp/Fconst / ((theta - 1.0) * theta)

  ! Exchange Current Density
  i_ex_curr    = Fconst * k_rxn * c0_r**alpha_a * c_x_r**alpha_c * (max_Li_conc - c_x_r)**alpha_a
  dex_curr_dc0 = Fconst * k_rxn * alpha_a * c0_r**(alpha_a - 1) * c_x_r**alpha_c * (max_Li_conc - c_x_r)**alpha_a
  dex_curr_dcx = Fconst * k_rxn * c0_r**alpha_a * ( alpha_c * c_x_r**(alpha_c - 1) * (max_Li_conc - c_x_r)**alpha_a + &
                                              &  c_x_r**alpha_c * alpha_a * (max_Li_conc - c_x_r)**(alpha_a - 1) )

  ! Butler-Volmer Equation
  eta        = Phi_1_r - Phi_2_r - U_ocp
  deta_dPhi1 = 1.0
  deta_dPhi2 = -1.0
  deta_dc0   = -dOCP_dc0
  deta_dcx   = -dOCP_dcx
  ...
  di_BV_deta  = i_ex_curr * ( alpha_a * Fconst/(Rigc*Temp) * EXP(alpha_a * Fconst * eta / (Rigc * Temp)) &
                          - (-alpha_c * Fconst/(Rigc*Temp)) * EXP(-alpha_c * Fconst * eta / (Rigc * Temp)) )
  di_BV_dPhi1 = di_BV_deta * deta_dPhi1
  di_BV_dPhi2 = di_BV_deta * deta_dPhi2
  di_BV_dc0   = di_BV_deta * deta_dc0
  di_BV_dcx   = di_BV_deta * deta_dcx

  spec_a = volumetric_surface_area
  u_Li   = diff_Li / (Rigc * Temp)
  u_PF6  = diff_PF6 / (Rigc * Temp)
  ...
  if (j == 1) then                              ! West Side Boundary Conditions
    ...
    c0 = cbulk
    rj(1,1) = 1.0
    smG(1)  = cbulk - cprev(1,1)

    ! (1-eps)dc/dt = -a * i_BV/F
    rj(2,1) = -spec_a*di_BV_dc0/Fconst
    rj(2,2) = -spec_a*di_BV_dcx/Fconst - (1.0 - porosity) * 1.0/delT
    rj(2,3) = -spec_a*di_BV_dPhi1/Fconst
    rj(2,4) = -spec_a*di_BV_dPhi2/Fconst
    smG(2)  = +spec_a*i_BV/Fconst

    ! -(1-eps) * simga * dV/dx = 0 ! no electron flux at the separator
    dE(3,3) = -(1.0 - porosity)*sigma
    smG(3)  =  0.0 - dE(3,3)*dcdxE(3)

    ! Phi_2 = 0 at separator
    rj(4,4) = 1.0
    ...
    ! No flux of c0 at the current collector
    dW(1,1) = -porosity*1.5*diff_Li
    fW(1,1) = -porosity*1.5*z_Li*u_Li*Fconst*dcdxW(4)

    dW(1,4) = -porosity*1.5*z_Li*u_Li*Fconst*cW(1)
    fW(1,4) = 0.0
    ...
    ! -(1-eps) * sigma * dV/dx = i_applied      at the current collector
    dW(3,3) = -(1.0 - porosity)*sigma
    smG(3)  =  applied_current_A - dW(3,3)*dcdxW(3)

    ! solution current (ionic current) is zero at current collector
    dW(4,1) = -porosity**1.5*Fconst*(z_Li*diff_Li + z_PF6*diff_PF6)
    fW(4,1) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*dcdxW(4)

    dW(4,4) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*cW(1)
    fW(4,4) = 0.0

    smG(4)  = 0.0 - dW(4,1)*dcdxW(1) - fW(4,1)*cW(1)
  ...
  !-----------------------------------------------------------------------------
    ...
    dW(1,1) = -porosity**1.5*diff_Li
    dE(1,1) = -porosity**1.5*diff_Li
    fW(1,1) = -porosity**1.5*z_Li*u_Li*Fconst*dcdxW(4)
    fE(1,1) = -porosity**1.5*z_Li*u_Li*Fconst*dcdxE(4)

    dW(1,4) = -porosity**1.5*z_Li*u_Li*Fconst*cW(1)
    dE(1,4) = -porosity**1.5*z_Li*u_Li*Fconst*cE(1)
    fW(1,4) = 0.0
    fE(1,4) = 0.0

    rj(1,1) = spec_a*di_BV_dc0/Fconst*delX(j) - porosity/delT*delX(j)
    rj(1,2) = spec_a*di_BV_dcx/Fconst*delX(j)
    rj(1,3) = spec_a*di_BV_dPhi1/Fconst*delX(j)
    rj(1,4) = spec_a*di_BV_dPhi2/Fconst*delX(j)

    smG(1)  = -spec_a*i_BV/Fconst*delX(j) &
...
! dc/dt - (di_rxn_dc * delC + di_rxn_dcs * delCs + di_rxn_dphi_1 * delPhi_1 + di_rxn_dphi_2 * delPhi_2)/Fconst = i_rxn/Fconst
    ...
    rj(2,1) = -spec_a*di_BV_dc0/Fconst
    rj(2,2) = -spec_a*di_BV_dcx/Fconst - (1 - porosity) * 1.0/delT
    rj(2,3) = -spec_a*di_BV_dPhi1/Fconst
    rj(2,4) = -spec_a*di_BV_dPhi2/Fconst
...
!  - delX*(di_rxn_dc * delC + di_rxn_dcs * delCs + di_rxn_dphi_1 * delPhi_1 + di_rxn_dphi_2 * delPhi_2) = -sigma * dcdx_W - (-sigma * dcdx_E) + delX*i_rxn
    ...
    rj(3,1) = -spec_a*di_BV_dc0*delX(j)
    rj(3,2) = -spec_a*di_BV_dcx*delX(j)
    rj(3,3) = -spec_a*di_BV_dPhi1*delX(j)
    rj(3,4) = -spec_a*di_BV_dPhi2*delX(j)

    smG(3)  = +spec_a*i_BV*delX(j) &
...
!       kappa = Fconst**2*(z_Li**2*u_cat*conc_cat + z_an**2*u_an*conc_an)
!       kappa = Fconst**2*(z_Li**2*u_cat + z_an**2*u_an)*conc

    dW(4,1) = -porosity**1.5*Fconst*(z_Li*diff_Li + z_PF6*diff_PF6)
    dE(4,1) = -porosity**1.5*Fconst*(z_Li*diff_Li + z_PF6*diff_PF6)
    fW(4,1) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*dcdxW(4)
    fE(4,1) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*dcdxE(4)

    dW(4,4) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*cW(1)
    dE(4,4) = -porosity**1.5*Fconst**2*(z_Li**2*u_Li + z_PF6**2*u_PF6)*cE(1)
    fW(4,4) = 0.0
    fE(4,4) = 0.0

    rj(4,1) = +spec_a*di_BV_dc0*delX(j)
    rj(4,2) = +spec_a*di_BV_dcx*delX(j)
    rj(4,3) = +spec_a*di_BV_dPhi1*delX(j)
    rj(4,4) = +spec_a*di_BV_dPhi2*delX(j)

    smG(4)  = -spec_a*i_BV*delX(j) &
    &         - (fW(4,4)*cW(4) + dW(4,4)*dcdxW(4)) &
    &         + (fE(4,4)*cE(4) + dE(4,4)*dcdxE(4))
