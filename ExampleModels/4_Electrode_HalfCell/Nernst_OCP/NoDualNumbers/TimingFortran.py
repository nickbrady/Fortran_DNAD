...
c_rates = [3, 5, 10]#[10, 5, 4, 3, 2, 1]
for c_rate in c_rates:

    _source_tran = _sorted_fortran_template#'{}/{}'.format(cwd, _sorted_fortran_template)
    ...
    p = call(['gfortran', '-fdefault-real-8', '-O3', fortran_values])   # compile fortran

    timings = []

    for ll in tqdm(range(30)):
        t0 = time.time()
        p = call(['./a.out'])                                           # execute fortran
        timings.append(time.time() - t0)
    print('C-rate C/{}'.format(c_rate))
    print('Average time (seconds) per execution: ', np.mean(timings), ' +/- ', np.std(timings))
    print(timings)
    print('')
