...
module user_input
  ...
  integer, parameter :: NJ = 402                          ! Number of mesh points
  ...
  real :: diff_low_conc  = 1.0e-18       ! Lithium diffusion coefficent [cm2/s]
  real :: diff_high_conc = 5.0e-20       ! lithium diff coeff at high Li conc
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it.EQ.1) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Cycle', 'State', 'Time', 'Equiv',    'Potential', 'conc_x'
      write(*, header_fmt) '#', 'CDR',  'hours', 'LixFe3O4', 'Volts'  ,      'LixFe3O4'
    end if
    ...
    write(*, data_fmt) cycle_number, state, t_write, Equiv, Phi_1, c_x_Li
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Cycle', 'State', 'Time', 'Equivalence', 'Voltage', 'Position', 'Solid_Conc'
      write(56, header_fmt) '#',     'CDR',  'hours', 'LixFe3O4' , 'Volts'    , 'nm'      , 'LixFe3O4'
                                                    !           !
    end if                                          !           !
    ...
    do j = 1, NJ                                    !           !
      c_x_Li = cprev(1,j) / max_Li_conc * 8.0        ! Li_xFe_3O_4
                                                    !           !
      write(56, data_fmt) cycle_number, state,    t_write,    Equiv,       Phi_1,     xx(j)*1e7,    c_x_Li
...
contains
  ...
  function DIFF_COEF(c_vars_dual) result(diff_)
    type(dual)                            :: diff_, xA
    type(dual), dimension(N), intent(in)  :: c_vars_dual

    xA = c_vars_dual(1)/max_Li_conc * 8.0

  	diff_ = diff_low_conc * ( 1.0 - TANH(1.5*(xA)**2.) ) + diff_high_conc
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  ...
  function Boundary_EAST (c_vars_dual, dcdx_vars_dual) result(BC_EAST_)
    ...
    BC_EAST_(1) = flux_temp(1) - applied_current_A / Fconst       ! N_x = i / F
...
! *****************************************************************************
  if (experimental_program == 'C_200_Discharge_Rest_Charge') then
    ...
    discharge_time = 1.0/c_rate * ( 1.5 / 8.0) * 3600.0 !
