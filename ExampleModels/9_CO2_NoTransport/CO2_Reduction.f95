...
module user_input
  ...
  integer, parameter :: Numbertimesteps = 200     ! Number of time steps
...
contains
  ...
  subroutine write_to_screen(it)
    ...
    if (it.EQ.1) then       ! write the headers on the first entrance into write all voltage
      write(*, header_fmt) 'Time',  '[CO2]',  '[H+]', '[HCO3-]', '[CO3-2]', '[OH-]', '[K+]', 'Phi_2'
      write(*, header_fmt) 'hours', 'mol/L',  'mol/L', 'mol/L',  'mol/L', 'mol/L',  'mol/L', 'Volts'
    end if
    ...
    write(*, data_fmt) t_write,    c_CO2,  c_H, c_HCO3, c_CO3, c_OH, c_K, Phi_2
...
!           write the headers on the first entrance into write all voltage
      write(56, header_fmt) 'Time',  'Position', '[CO2]' , '[H+]',  '[K+]', 'Soln_Pot'
      write(56, header_fmt) 'hours', 'um'      , 'mol/L' , 'mol/L', 'mol/L', 'Volts'
                                                    !
    end if                                          !
                                                    !
    do j = 1, NJ                                    !
      ...
      Phi_2 = cprev(4,j)
                                                    !           !
      write(56, data_fmt) t_write,    xx(j)*1e4,    c_CO2,       c_H, c_K, Phi_2
...
! (4)     0 = sum_i (z_i * c_i)       ! electroneutrality
  ...
  function RXN(c_vars_dual) result(Rxn_)
    ...
    Rxn_(2) = 0.0
    Rxn_(3) = 0.0
    Rxn_(4) = c_HCO3 - K_6 * c_CO3 * c_H
    Rxn_(5) = c_CO2 - K_5 * c_H * c_HCO3
...
! dc_n%x = 0.0      -->   dc        -->           BC_WEST_(2) = dc_n/delT - rxn
  function Boundary_WEST (c_vars_dual, dcdx_vars_dual) result(BC_WEST_)
    ...
    real :: Phi_1, U_1, U_3, k_1, k_3, alpha_c1, alpha_c3
    ...
    U_1 = -0.103
    eta_1 = U_1 - Phi_1 - Phi_2
    eta_1 = Phi_1 - Phi_2
    k_1 = 5.8e-9
    alpha_c1 = 0.25

    U_3 = 0.0
    eta_3 = U_3 - Phi_1 - Phi_2
    eta_3 = Phi_1 - Phi_2
    k_3 = 8.3e-13
    alpha_c3 = 0.20
...
program unsteady
  ...
  integer :: it, j
  real    :: abs_error, rel_error, tol = 1e-12
