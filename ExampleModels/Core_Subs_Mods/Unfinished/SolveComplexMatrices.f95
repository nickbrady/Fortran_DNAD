...
program unsteady
  ...
  integer :: it, j, row, col

  complex, dimension(2, 2)     :: A_test
  ...
  do row = 1, 2
    do col = 1, 2
      A_test(row,col) = complex(float(row), float(col))
    end do
  end do

  print*, A_test
  print*, real(A_test)    ! REALPART(A_test)
  print*, aimag(A_test)   ! IMAGPART(A_tes)
