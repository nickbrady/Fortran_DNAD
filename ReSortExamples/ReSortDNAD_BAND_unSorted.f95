module user_input
  ...
  real :: k_rxn = -1.0e-3
	...
	real :: xmin  = 0.0					 ! System domain [xmin, xmax]
  real :: xmax  = 1.0          ! Electrode Thickness (cm) [1 um = 1d-4 cm]
  real :: cbulk = 1.0          ! Electrolyte Concentration [mol/cm3]

  CHARACTER(LEN=1) :: state = 'D'

  ! define the system Geometry : Rectangular, Cylindrical, Spherical
  character(len=65) :: geometry = 'Spherical'
...
! ******************************************************************************
  ...
  END FUNCTION dcdx_to_dual
	! consider moving these functions because they do not need to be modified by the user
	! 	** c_to_dual
	! 	** dcdx_to_dual
	!		** Control_Volume
...
! Incorporates DNAD which enables the use of automatic Differentiation to
! linearize the PDE's
...
subroutine initial_condition()
  use user_input
  use variables, only: cprev, xx, delX
...
!*******************************************************************************
! Below are modules and subroutines that only in very rare situtions need to be
! modified by the user.
! modules: variables, dnadmod
! subroutines: auto_fill, ABDGXY, MATINV, BAND
!*******************************************************************************
...
subroutine auto_fill(j)
  ...
  smG = 0.0
	...
	! ****************************************************************************
	! define control-volume edge concentrations: cE, cW
	! and gradients: dcdxE, dcdxE
	! ****************************************************************************
...
subroutine ABDGXY(j)
  ...
  ! can be found in Newman appendix C.
      use user_input, only: N, NJ
      use GOV_EQNS, 	only: alphaE, alphaW, betaE, betaW, dE, dW, fE, fW, rj, &
														& smG
